...
- **`flutter_foreground_task` joins the legacy-KGP list.** The Android build now warns for three plugins — `battery_plus`, `cloud_functions` and `flutter_foreground_task` — where open item 14 names only the first. Builds today; a future Flutter will refuse it.
