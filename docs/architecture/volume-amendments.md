...
| 131 | **Mission 7.5's F5 exists only in conversation and cannot be reconstructed from the repository** | 7.5 addressed five verification gaps. Four produced amendments — A-216 (cursor round trip), A-217 (404 copy on device), A-218 (F4, inconclusive), A-219 (device identity). **F5 produced nothing, and nothing anywhere says what it was.**<br><br>Searched exhaustively at Mission 7.10: `docs/` names Mission 7.5 in one file, no document enumerates its gaps, and every other `F5` in the repository belongs to a different mission — the F-numbers are per-mission and reused, which is what let this hide.<br><br>**This is open item 115's shape, recurring.** That item records a CI batch that *"existed only in conversation, which is the failure this register exists to prevent."* The same thing then happened to a verification gap, in the mission whose entire purpose was closing verification gaps. **Needs the project owner to say what F5 was, or it is lost.** | item 115, A-216 through A-219, Mission 7.5 |
...
**So F5 exists only in conversation**, which is open item 115's shape — *"the batch existed only in conversation, which is the failure this register exists to prevent"* — recurring, undetected, **inside the mission whose entire purpose was closing verification gaps.** Open item 131; it needs the project owner to say what F5 was or it is lost.
