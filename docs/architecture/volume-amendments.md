...
| 49 | **The `Environment consistency` job has never been capable of passing — broken in the commit that created it (`63d97e5`)** | **Found 2026-08-16 by Mission 4.3.7, in the pipeline's first real run.** `ci.yml`'s workflow-level `defaults.run.working-directory: mobile` applies to every job and no job overrides it, so this job's Python opens repo-root-relative paths one directory down: `infrastructure/aws/config/environments.json` becomes `mobile/infrastructure/...`, and `mobile/lib/app/config/app_environment.dart` becomes `mobile/mobile/lib/...`. The doubled prefix is the tell. It dies on `FileNotFoundError` at the first `open()`, **before evaluating a single assertion** — so it has never once compared Dart, JSON and shell. **The environment model it guards is in fact correct**: run from the repo root against the committed blobs the job exits 0 — dev/staging/prod agree across all three languages, buckets follow ADR-011's slug rule, region is `ap-south-1` everywhere. Verified in both directions (root: exit 0; from `mobile/`: crash). `git log -S` places the job and the `working-directory` default in the same commit. **Not a Mission 4.3 defect and not fixed here** — see item 51. | Mission 4.3.7, `63d97e5` |
| 50 | **SECURITY — the `Secret scan` job reports green while scanning 79% of the repository, and 0% of `infrastructure/`** | **Found 2026-08-16 by Mission 4.3.7.** Same root cause as item 49: the job inherits `working-directory: mobile`, and both of its mechanisms — `git ls-files` and `git grep -- .` — are scoped to the current directory. It therefore sees **444 of 561** tracked files and **zero** files under `infrastructure/`, which is exactly where the AWS configuration lives. `docs/`, `.github/` and repo-root files are also unseen. **A committed `.pem`, `.env`, private key or AWS access key under `infrastructure/aws/` would pass this scan today.** This is more dangerous than item 49: a red job is visible, a hollow green one is not. It is item 41's *"a green check over an empty set is not evidence"* in its most literal form — the check ran, passed, and was pointed at the wrong subtree. **Reported, not self-fixed**, per the standing rule that security findings are surfaced rather than quietly patched (cf. items 28, 45). No credential has been found; the finding is the absent coverage, not a disclosure. | Mission 4.3.7, ADR-016 |
| 51 | **Root cause of 49 and 50 — the workflow-level `working-directory` default versus any job that reasons about repo-root paths** | **A real design decision, deliberately not taken by Mission 4.3.8.** `defaults.run.working-directory: mobile` (`ci.yml`) is correct and convenient for the six Flutter jobs, which are all `mobile/`-relative, and silently wrong for every job whose subject is the repository. Three coherent fixes, and they are not equivalent: **(a)** give the repo-scoped jobs `working-directory: .` and leave the default alone — smallest diff, but the trap stays armed for the next job someone adds; **(b)** remove the default and make every job state its own directory — explicit everywhere, noisier, and touches all nine jobs; **(c)** keep the default and rewrite the offending jobs' paths to be `mobile/`-relative — smallest conceptual change, but it encodes the repo layout into a security scan's path strings, which is how item 50 happened. Choosing needs a view on whether CI jobs are Flutter-first with exceptions, or repository-first with a Flutter convenience — an ADR-019/ADR-021 question rather than a YAML edit. **Whichever is chosen, the fix must be verified by deliberate breakage** (a planted secret under `infrastructure/`, a planted drift in `environments.json`), because both jobs currently prove that passing is not evidence of checking. | Mission 4.3.7 |
