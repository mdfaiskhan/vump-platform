...
**CLOSED 2026-08-24 by [[ADR-053]].** The pipeline now publishes a texture id, a display aspect ratio and a rotation — three numbers, no capability — and `CameraPreviewSurface` renders them, so FR-REC-03's preview exists and Mission 3.3's boundary is intact: the screen still holds nothing it could start or stop capture with. Verified on the same CPH2707 that reported the gap: rotation upright, aspect ratio natural, framing matching an ordinary camera app, Stop control unaffected. **A first attempt with a raw `Texture` rendered sideways and was abandoned** — the platform's display-rotation value is native-only, so no constant could have been right in every orientation; ADR-053's second amendment records why that made it impossible rather than merely imperfect. **[[item 147]] carries what this does not close**: Ch 9.4 target 5's battery figure was measured against a black screen and is owed a re-run with the preview live.<br><br>**Recorded here rather than as a new open item.** Mission 8.2 traced it as a suspected defect and found it already written down; opening a second row would split one gap across two records. What the field evidence adds is that the gap is **user-visible and self-reporting** — a Collector meets it on their first recording and describes it accurately without any prompting — which the written gap did not establish and which bears on how urgently the seam gets built.
...
| ~~36~~ | ~~**Backend session registration has no implementation — `SessionRegistrar` is a named, unowned port**~~ **CLOSED, 2026-08-21.** `SessionRegistrarImpl` was built at Mission 7.4 and `POST /v1/tasks/{taskId}/sessions` is deployed and answering. The item's own test — *"until this lands the pipeline cannot upload anything"* — is settled the only way it could be: a chunk was recorded on a CPH2707, uploaded to real S3 and confirmed in Aurora, and the chain then ran at scale across a 38-part chunk and a session spanning two chunks. **It was found still open by Mission 7.12, four sub-missions after the work that closed it** — the mirror of item 130, which was a debt recorded where nobody scanning live state would look. This was a resolution that never reached live state at all. | **Owed to whichever mission builds `features/projects_tasks/`.** Ch. 5.10 §1 step 1's URL needs a backend session id from `POST /v1/tasks/{id}/sessions`, which needs a `task_id`. `sessionRegistrarProvider` throws; a fake satisfies it in tests only. **Until this lands the pipeline cannot upload anything**, which is why 4.2's verification is against the fake rather than a real endpoint. | A-071, Ch. 4.6 §4 |
| ~~37~~ | ~~**Guard 1 refuses 100% of chunks recorded on a device**~~ **CLOSED, 2026-08-21.** The item recorded correct behaviour waiting on three causes — items 1, 5 and 11 — because four of `MetadataIdentity`'s five fields carried the unsourced sentinel. All three are resolved: `features/projects_tasks/` exists, the `collector_id` inversion was corrected at Mission 7.4 (A-206), and a `device_id` source exists. Guard 1 now passes real chunks, proved by the same device upload that closed item 36. **Also found still open by Mission 7.12.** | **Correct behaviour, not a defect — and the reason it fires is items 1, 5 and 11.** Four of `MetadataIdentity`'s five fields carry the unsourced sentinel, so no real chunk can be uploaded until `features/projects_tasks/`, the `collector_id` inversion and a `device_id` source all exist. Named here so nobody reads an empty upload log as a broken pipeline. | A-068 Guard 1, A-062 §1 |
...
| 125 | **`features/auth/application` coverage fell 93.8% → 85.2% and nobody knows why** | Measured in Mission 7.7 against Mission 2.8's figure, twelve missions apart. The layer is 81 instrumented lines, so **8.6 points is roughly seven lines** — small in absolute terms and unexplained in every other sense.<br><br>**Nothing in Mission 7.7 touched this layer.** The drop accumulated across 7.1–7.6, most plausibly from `auth_notifier` gaining session and token-refresh paths that ADR-048's authorizer work introduced. That is a guess, and it is written here as one.<br><br>**The finding is the invisibility, not the number.** A dated table records what was true when it was written and reports nothing afterwards; this drift was silent for twelve missions and surfaced only because a second measurement was taken beside the first. Investigating it means running coverage on the layer and reading which lines are new and unhit — cheap, and nobody has. | testing-standards.md §17, A-028, Mission 2.8 |
| 126 | **Nothing checks that two components agree on a value both must resolve the same way** | A-230 is the third instance: the client read `role` from the Firebase claim while the authorizer read it from the `users` table, and each was internally correct. A-222 was a GCP attribute mapping against its IAM binding; A-228 was `package.json` against `package-lock.json`.<br><br>**The first two were caught by an apply and by CI. This one was caught by reading**, because nothing executes the comparison — the two sources agree in practice today and would diverge silently on the day they stopped.<br><br>**No general check exists and one may not be worth building.** What the three share is a pair that must agree with no single place asserting it; what they do not share is a mechanism, so the practical form is a habit at review — when a value is resolved in two places, ask which is authoritative and make the other read it. | A-230, A-222, A-228, ADR-048 |
| ~~127~~ | ~~**Four backend files are exempt from the coverage per-file floor**~~ **CLOSED BY MEASUREMENT, 2026-08-22.** All four cleared their targets at Mission 8.1 and `EXEMPT` in `check-coverage.mjs` is now empty: `authorizer.ts` and `caller.ts` to **100%**, `runner.ts` to **98.21%** against 70, `firebase.ts` to **100%** against 40. Backend lines went 82.94% to **94.14%** and 71 tests were added.<br><br>**The mechanism the row describes is kept even though the list is empty** — an exemption is still a floor of its own, so anything added later fails the gate the moment it clears its own target. What closed is the debt, not the guard.<br><br>**The row's own note that `firebase.ts`'s 40 was *"different in kind"* turned out to be the interesting part and was wrong** — see [[A-236]]. | A-231. `runner.ts` 0% (target 70), `caller.ts` 0% (80), `authorizer.ts` 9% (80), `firebase.ts` 33% (40). **Every one is testable** — none is exempt because it cannot be tested. Mission 7.9 chose not to take on `runner.ts`'s 251 lines and `caller.ts`'s Data API fixtures in one session, and said so.<br><br>**The list cannot quietly become permanent.** An exemption is a floor of its own: a file rising above its target fails the gate until its entry is removed, proven by deliberately breaking that branch.<br><br>**`firebase.ts`'s 40 is different in kind** from the other three — its remainder is the live token exchange, which no unit test can reach. The other three are effort, not ceiling. | A-231, ADR-045, `scripts/check-coverage.mjs` |
| 128 | **`process.env[k] = undefined` sets the string "undefined" and does not unset** | A-231. Three tests asserting a missing environment variable is refused passed **vacuously** — `requireEnv` received `"undefined"`, a non-empty string, and never threw. `Reflect.deleteProperty` is the unset.<br><br>**Caught only because they failed first for an unrelated reason.** Written the other way round they would have passed and proven nothing, which is the shape this register keeps recording.<br><br>**The idiom is in this repository's `afterEach` blocks** and is not wrong there — each `beforeEach` overwrites the key. It is wrong anywhere a test depends on a variable being ABSENT, and nothing distinguishes the two uses at a glance.<br><br>**This entry is the instance, not the class.** The general problem — a test that passes while asserting nothing, with no mechanism to detect it — is open item 129. Fixing these three did not narrow that. | A-231, A-212, item 129 |
| 129 | **Nothing detects a test that passes without asserting anything** | Open item 128's three vacuous passes were caught **by accident** — they failed first for an unrelated reason, and the fix revealed they had never been testing what they claimed. Written the other way round they would have been green from the start.<br><br>**So the instance is fixed and the class is not.** This shape can exist anywhere in either suite right now with no accidental trigger to expose it, and 1,390 tests across the two trees is far past what re-reading finds. A green suite is the strongest evidence this project has and it is exactly the evidence this defect hides inside.<br><br>**The known members so far**: A-212 (a mock that could not reject), A-224 (a fake that accepted any payload), A-231's three env-var tests, and Mission 7.8's write banner, which had no test at all rather than a vacuous one. Different mechanisms, one outcome.<br><br>**Sampled at Mission 7.10 and NARROWED, not closed.** Twenty tests, chosen deliberately from the thinnest-history end of both trees and excluding everything 7.6-7.9 wrote: **18 killed, 2 survived** (A-232). One was a coverage gap, one a tautological assertion comparing a module to itself; both are fixed. **Neither would have been found by reading** — both read as careful and one is. That is 90% at the end most likely to be weak, and ~1,370 tests still unexamined.<br><br>**Wants a deliberate sweep, not vigilance.** Mutation testing is the tool that answers it directly — change the code, expect a test to fail, and any surviving mutant names a test proving nothing. Candidates for the backend include Stryker; the Dart side has less mature options and may need the cheaper version, which is deleting an assertion's subject and checking the test still fails. Its own mission. | A-231, item 128, A-212, A-224 |
| 130 | **A-218's F4 is open and appears in no register a reader would scan** | The foreground service surviving a genuine, timestamped screen-off window is **still unproven**. A-218 records two attempts that established nothing — the capture file never existed — and A-215's qualifier stands unchanged.<br><br>**It was recorded in an amendment and nowhere else.** Amendments are history; this register is live state. Someone asking "what is still unverified" reads the second and would not find it, which is how it survived Missions 7.6 through 7.9 without being picked up.<br><br>**The rule A-218 leaves is cheap and specific**: confirm the capture exists and is growing before starting the run — one `dir`, then a repeat two seconds later. Every attempt so far would have ended in the first thirty seconds. The `chunk-hash-probe` Lambdas and `probe.tf` are still deployed pending this. | A-218, A-215, Mission 7.5 |
| ~~131~~ | ~~**Mission 7.5's F5 exists only in conversation**~~ **CLOSED AS LOST, 2026-08-21.** 7.5 addressed five verification gaps. Four produced amendments — A-216, A-217, A-218, A-219. **The fifth produced nothing, and nothing anywhere says what it was.**<br><br>Searched exhaustively at Mission 7.10: `docs/` names Mission 7.5 in one file, no document enumerates its gaps, and every other `F5` in the repository belongs to a different mission — the F-numbers are per-mission and reused, which is what let it hide.<br><br>**Closed as lost rather than left open, and the distinction is deliberate.** An open item asserts that someone is expected to resolve it. Neither the project owner nor the assistant can recall F5's subject, and four missions have passed without it surfacing — leaving it open would imply a recovery that nobody is going to perform. **Reopen if it turns up in a record outside this repository**; do not leave it standing as a task.<br><br>**What survives is the lesson, not the item.** See the throughline note below. | **Closed — lost** | item 115, A-216 through A-219, A-232 |
...
| ~~137~~ | ~~**An interrupted upload leaves its chunk stuck in Uploading forever — no retry, no failure, no recovery**~~ **CLOSED 2026-08-23 — FIXED AND HARDWARE-VERIFIED, not merely unit-tested.**<br><br>**Fixed by [[ADR-052]]**, which reconciles rows still reading `uploading` when the dispatcher starts, spending one of Chapter 5.13 §2's attempts per process death so repeated kills converge on `failed` rather than retrying for ever. **That ADR's own first build did not work**, and its `Correction, 2026-08-23` section records why: reconciliation wrote the right row and nothing woke the dispatcher, because `watchQueue()` attaches its `watchLazy()` listener only after its first read and the write landed in that gap. The remedy was to make reconciliation read the queue and update the dispatcher itself rather than wait for an emission that may never come.<br><br>**The database state was always correct — proved by experiment, not argued.** With a 369 MB chunk apparently stuck, an unrelated 15-second recording was made. Its write produced an emission, the dispatcher woke, and it immediately claimed **both** chunks; the stranded one uploaded and verified at 22:05:21. That isolated the defect to *nothing tells the dispatcher to look again* and ruled out data loss, a dead subscription, and any fault in `claimNext`'s eligibility filter.<br><br>**Closing run, build `e1b53d4`, 2026-08-23.** Chunk `ae6e8d43-6922-4b59-b43b-7a1d04e320d0`, **275,896,765 bytes**, `am force-stop` **4.2 s** into a live multipart transfer. On relaunch: queue claimed at 22:27:25.561, reconciliation at **22:27:25.714**, first `PUT` at **22:27:27.194** — **1.48 s later** — and `Chunk ae6e8d43 uploaded.` at 22:28:14.522. **S3 21 to 22**, the object **byte-exact** against the file on disk. Relaunch to first PUT **2.99 s**; relaunch to complete **50.3 s**.<br><br>**No second write, no intervention** — which is the whole point, because the previous attempt recovered only when an unrelated recording happened to emit. Confirmed on screen by the project owner: uploading automatically, bar and percentage advancing, upload completed. No Retry Chunk button, correctly, since the chunk was never `failed`. **Ch 9.8 scenario 2 passes on this build**; its earlier FAIL stands as accurate documentation of the pre-fix state rather than being overwritten.<br><br>**What is closed and what is not.** The stranding is fixed and verified on hardware. The **retry-gating finding below is not made obsolete** — `chunk_row.dart` still offers Retry Chunk only for `failed`, which remains right for the manual path and is why the fix had to be a reconciliation rather than a widened gate. [[item 141]] records that this ADR's first five tests passed while the code was broken on a device. [[item 139]] stays open on its own merits; it never blocked this item's mechanism, which was settled by reading source. | **Found on a CPH2707 at Mission 8.2, 2026-08-22, and confirmed by the project owner's direct observation of the screen.**<br><br>**The sequence, from a 64 MiB logcat capture.** Two recordings finalized at 21:33:42.88 (95,472,915 bytes) and 21:35:03.35 (63,299,951 bytes). Both registered successfully — `POST /v1/tasks/{id}/sessions` → **201**, `POST /v1/sessions/{id}/chunks` → **201** — and both S3 PUTs were issued, at **21:33:47.373** and **21:35:06.744**. Neither PUT logged a response. Each was cut short by an engine restart ([[item 138]]).<br><br>**Then the dispatcher said `Network restored — claiming the queue.` at 21:35:12.479 and never spoke again.** Twelve minutes later: no PUT, no error, no `released back to the queue`, no state change. Verified against the device's own buffer independently of the capture — **0 flutter lines after 21:35:12.479**.<br><br>**Chapter 5.13's schedule is 5s, 10s, 20s, 40s, 80s, capped at 5 minutes. Twelve minutes is 2.4x the cap.** This is not a retry waiting its turn; no retry is scheduled at all.<br><br>**The project owner's screen, at the same moment**: both sessions read **"Uploading"** — not Queued, not Failed — and the notification reads **"Upload in progress — Uploading 1 of 2 chunks"**. The foreground service is alive (`isForeground=true`, `foregroundId=5110`), so the notification is truthful about the service and wrong about the work.<br><br>**HYPOTHESIS — the orphaned row. Persistence CONFIRMED, mechanism UNCONFIRMED.**<br><br>A force-stop at 21:57:35 removed the process and all service records; a manual relaunch at 21:57:55 produced pid 26459, a fresh Dart VM and a fresh Isar open. The dispatcher claimed the queue at 21:57:59.495 and made no PUT in the following two minutes. **Across the session: five starts, five claims, zero uploads**, with both chunk files intact and S3 unchanged at 11.<br><br>**So the state that blocks these chunks survives process death.** It is persisted, and restarting is not a remedy — that half is established by experiment rather than inference.<br><br>**The specific mechanism is not.** That each row sits at status `uploading` while the dispatcher's claim query selects only `queued` remains the only hypothesis fitting every observation, and it has not been read. Confirming it requires querying the chunk rows in local Isar and in the backend `chunks` table.<br><br>**The Data API read was attempted at Mission 8.2 and is blocked.** `vump-dev-operator` hangs on an MFA assume-role this non-interactive terminal cannot answer; `faisal-dev` authenticates but is refused by identity policy on both `rds:DescribeDBClusters` and `secretsmanager:ListSecrets`; `w32time` cannot be started to rule out clock drift, and drift was not the cause. Unresolved within this sub-mission's scope.<br><br>**Nothing is lost and nothing is duplicated.** Both chunk files are intact on disk at full size, S3 holds exactly one new object against a baseline locked before the test (11 vs 10), and BR-08 is upheld — storage cleanup removed only files whose uploads were confirmed, verified by matching 41.3 MB decimal to the 41,290,848-byte object.<br><br>**NOT evidence for F4 / item 130.** The screen was on and the app was in the foreground throughout. This says nothing about a foreground service surviving a screen-off window, and must not be read as though it does. <br><br>**EVIDENCE ADDENDUM, 2026-08-23 — a clean upload while the two stayed stuck.** At 01:40 the project owner recorded a short clip on the same device, same account, same Task. Session `1ddbb5a9`, chunk `834bdec4`, **735,489 bytes**. It registered, uploaded and verified end to end in **three seconds**: `PUT` → 200 at 01:40:57.780, metadata `POST` → 201, `PATCH {status: complete}` → **`{status: complete, verified: true}`**, and `Chunk 834bdec4 uploaded.` at 01:41:00.274. S3 went 11 → **12**.<br><br>**The same process had claimed the queue nineteen seconds earlier** — `Network restored — claiming the queue.` at 01:40:41.415 — and both stuck chunks were on disk throughout, unchanged at 95,472,915 and 63,299,951 bytes.<br><br>**So the dispatcher is not broken.** It claimed, then uploaded a new chunk minutes later, in the same process, over the same network, against the same backend. **Two specific rows are invisible to it** while everything around them works. That narrows the fault from *the upload path* to *these two records*, which is what the orphaned-row hypothesis predicts and a broken dispatcher does not.<br><br>It also bears on [[item 138]] without settling it: the new chunk was **735 KB and finished in 379 ms**, far too fast to be interrupted, and no engine restart occurred. Consistent with that item's size correlation — 95 MB and 63 MB were aborted, 735 KB was not — and still short of proof. <br><br>**THE MANUAL REMEDY EXISTS AND IS UNREACHABLE IN EXACTLY THIS STATE.** FR-UPL-07's *"Retry Chunk"* action is implemented: `collector_sessions_screen.dart` passes `onRetry` to every row, and it calls `uploadQueueNotifierProvider.retry(chunkId)` → **`chunkQueueSourceProvider.requeue(chunkId)`** — precisely the operation that would return an orphaned row to the queue.<br><br>**But `chunk_row.dart:123` gates the button behind `if (isFailed)`**, where `isFailed = chunk.status == ChunkUploadStatus.failed`. A chunk stuck at `uploading` is never `failed`, so **the button never renders for it**. The one state in which a Collector most needs manual retry is the one state that does not offer it, and the remedy sits one status value away from the problem it would fix.<br><br>This also cost the investigation its cheapest confirmation: invoking `requeue` on a stuck chunk and watching it upload would have come close to proving the orphaned-row mechanism, and there is no path to invoke it from the UI.<br><br>**CLOSING NOTE, 2026-08-23.** Live reproduction cleared on-device at the project owner's decision, to restore normal app function — this was test data, not preserved production evidence. `pm clear` was refused by the device (`SecurityException: CLEAR_APP_USER_DATA`, an OEM restriction on this OnePlus), so the app was uninstalled and reinstalled from the same artefact instead; the wipe is therefore total. `device_id` changed from `9a897969-fe10-43fd-b950-2529add5d48e` to **`25e9d858-ad08-4faa-9221-9279b90fb3c0`**, which every future chunk now carries.<br><br>**All findings and log evidence already in this row stand unchanged.** The mechanism remains UNCONFIRMED and is now **unconfirmable via this specimen**; confirming it needs a fresh reproduction plus the credential path [[item 139]] blocks. **The symptom was cleared, not the defect** — an interrupted upload will produce the same stuck chunk again.<br><br>**REPRODUCED ON A CLEAN DEVICE, 2026-08-23 — Ch 9.8 scenario 2 — and this escalates the row.** Mission 8.2 ran the scenario this item was originally meant to inform, on the device wiped and reinstalled since the original specimen: fresh install, new `device_id` `25e9d858-ad08-4faa-9221-9279b90fb3c0`, empty queue, S3 baseline locked at 19.<br><br>**The kill was the canonical one and it landed inside the transfer.** Chunk `f42d29d6-1ad3-4e7a-a5e0-a488849cdfa5`, **520,658,550 bytes**, registered 18:22:49. An automated watcher issued `am force-stop` **4 seconds into the upload** — multipart PUTs went 112 to 116 with the completion count unchanged at 4 — at **18:23:00.290**. Process gone, zero service records. *(A first attempt is discarded rather than counted: its force-stop landed 9.5 s after the upload had already completed, so it never met the condition. The watcher exists because of that miss.)*<br><br>**Relaunch, then nothing.** pid 3965 at 18:23:41.620, auth 200, `Network restored — claiming the queue.` at **18:23:44.223** — and that is the last thing the dispatcher ever said. **12 minutes 33 seconds later: 0 PUTs, 0 uploads, 1 claim, 0 `released back to the queue`, 0 errors, 0 crash / ANR / `am_kill` / `lowmemorykiller` records, pid stable at 3965 with 1 live service record.** Chapter 5.13's cap is 5 minutes; this is **2.5x** it. No retry is scheduled.<br><br>**Nothing is lost and nothing is duplicated.** The chunk sits on disk at its full **520,658,550 bytes** and S3 is unchanged at **19** — **BR-08 upheld**.<br><br>**FOUR EXPLANATIONS DIE HERE, EACH BY THIS RUN ALONE.** **(1) Stale queue state** — wiped device, fresh install, new `device_id`, empty queue; the symptom returned regardless. **(2) The swipe-away confound** — this was a deliberate `am force-stop`, not an ambiguous recents swipe. **(3) [[item 138]] as the cause** — **zero engine restarts occurred**, see that item's correction. **(4) Chunk size** — 521 MB here against the original 95 MB, a **fourth** independent refutation.<br><br>**What survives is narrower and worse.** Any interruption of an in-flight upload persistently orphans its chunk: clean device, no engine restart, no crash, no memory pressure. The orphaned-row hypothesis — a row left at `uploading` while the claim query selects only `queued` — remains the only mechanism fitting every observation, and remains **unread**, because [[item 139]] still blocks every path to the database.<br><br>**THE UI MISREPORTS ON ALL THREE SURFACES, AND THE THIRD IS NEW.** Confirmed by the project owner's direct observation of the screen, not inferred from logs. The status pill reads **"Uploading"** against 0 PUTs since relaunch. The **Retry Chunk** button is **absent**, gated on `isFailed` while the chunk sits at `uploading`. And a progress bar **sweeps left to right** as though work were underway.<br><br>**That bar animates *because* nothing is happening.** `chunk_row.dart:117` renders `LinearProgressIndicator(value: progress?.fraction)`; a null `progress` yields a null `value`, which is Flutter's **indeterminate** animation. The comment immediately above it reasons that an indeterminate bar gets no screen-reader value because *"inventing '0 percent' would report a stall that is not happening"* — but here the stall **is** happening, and the visual affordance denies it while the screen reader stays silent about it. Chapter 2.10 §4 is satisfied in letter and inverted in effect: the one user who is told nothing misleading is the one using TalkBack.<br><br>**SEVERITY.** Previously one specimen carrying two confounds. Now: **reproducible on demand, on a clean device, from the most ordinary way a Collector can interrupt an upload — killing the app.** A 521 MB field recording was made permanently unrecoverable through the UI by a single force-stop. **Ch 9.8 scenario 2 is recorded FAIL.**<br><br>**SPECIMEN NOTE, 2026-08-23 — the second reproduction was also cleared, this time by hand through Settings.** The row above records the chunk as sitting on disk at 520,658,550 bytes; **it no longer does.** At **18:45:25.594** the events buffer records `am_clear_app_data_caller: [25234, 1000, com.vump.humanarchive.dev]`, followed by `am_kill … stop com.vump.humanarchive.dev due to clear data`, `pm_clear_app_data_caller` and `installer_clear_app_data_caller`. pid **25234** is **`com.android.settings`**, launched 4 seconds earlier at 18:45:21.627 into `InstalledAppDetails` via `android.settings.APPLICATION_DETAILS_SETTINGS`, then `SubSettings` at 18:45:24.035. That is App info, Storage, Clear data, tapped by hand. `firstInstallTime` and `lastUpdateTime` both remain 17:54:58, so the package was **not** reinstalled — only its data was erased. `device_id` moved from `25e9d858-…` to **`2e973a15-ccb4-42bd-a011-feba2fa14672`** and `vump.isar` is back to an empty 1,048,576 bytes.<br><br>**No finding in this row depends on the specimen surviving.** Every figure above was recorded from live observation with logcat timestamps while the state existed, and the S3 baseline of 19 is unchanged and independently checkable. **What is lost is only the ability to re-examine that particular chunk** — which mattered greatly for the first specimen and matters far less here, because unlike the original this reproduction came from a **written, repeatable procedure**: register a chunk, watch for the S3 PUT, issue `am force-stop` within seconds of the transfer starting, relaunch, and wait past Chapter 5.13's cap. **The defect can be produced again on demand.** It remains unfixed, and clearing the data removed the symptom rather than the cause for the second time.<br><br>**MECHANISM CONFIRMED, 2026-08-23 — from source, and the database read is no longer needed.** This row carried *mechanism UNCONFIRMED*, pending a read of the chunk rows that [[item 139]] blocks. **That read is not required.** Mission 8.2's fix trace established the orphaned-row hypothesis by construction, in five steps that are all visible in the code:<br><br>**1. Only one query claims work, and it excludes the state.** `IsarChunkStore.claimNext` filters `statusEqualTo(ChunkUploadStatus.queued.wireName)`. A row at `uploading` is not a candidate, however long it waits.<br><br>**2. Every exit from `uploading` is written by the pipeline that died.** The store writes a status in five places and three of them leave `uploading` — `deferAttempt`, and `_transition` for `markFailed` and `markComplete`. All three are in-process calls made by the running upload. A process death runs none of them.<br><br>**3. The manual escape is closed by an explicit guard.** `requeue` opens a guard that returns immediately unless the row exists and its status is exactly `ChunkUploadStatus.failed.wireName`, and the interface states the intent plainly: *"Ignores a chunk that is not currently `failed`."* **This sharpens the Retry Chunk finding already in this row**: the button being gated on `isFailed` was not the whole defect. Even had it rendered, the call behind it would have returned without doing anything. A UI-only fix would have produced a dead control.<br><br>**4. The absence of a recovery pass was intended, and said so three times.** `ChunkQueueSource` — *"there is no re-queue step at launch and no separate recovery pass. The rows already say what they are."* `UploadDispatcher.start` repeated it. And `shutDown` names the same failure from the other side, treating stranding as a hazard to avoid: *"abandoning them here would strand those rows in `uploading` with nothing left to release them."* The code knew what stranding meant; it had nothing to clear a row stranded anyway.<br><br>**5. Chapter 5.9 §3's premise is false, and that is the root cause.** The chapter offers the rows' persistence as what *"satisfies NFR-REL-04 directly: an app kill loses nothing"*. The rows do survive. **Surviving is not resuming** — a row reading `uploading` describes a transfer that no longer exists. **NFR-REL-04 is violated in precisely the scenario it is cited to protect**, by the reasoning meant to guarantee it.<br><br>**Persistence was already established by experiment; the mechanism is now established by construction. This row is CONFIRMED.**<br><br>**FIXED by ADR-052**, accepted 2026-08-23 and implemented the same day. The dispatcher reads one queue snapshot at startup and returns every row still at `uploading`, spending one of Chapter 5.13 §2's six attempts per death so the loop converges on `failed` rather than retrying forever. The fix is a reconciliation rather than a widened retry gate, because `requeue`'s guard is correct for the manual path — in a live app a row at `uploading` may genuinely be transferring, and re-queueing it would produce the double upload `claimNext` calls unrecoverable. A freshly launched process cannot hold a transfer, which is why startup is the one safe place to do this. | Mission 8.2, item 138, Ch 5.13, BR-08 |
| 138 | **The Dart engine restarts mid-upload, aborting the PUT — mechanism undetermined** | **Found on a CPH2707 at Mission 8.2, 2026-08-22, alongside [[item 137]], which it causes.**<br><br>**Twice, identically.** A large S3 PUT begins, and a few seconds later the Dart engine restarts — `Impeller … Using the Impeller rendering backend`, then `Vump Technologies 1.0.0+1 starting`, Firebase re-initialised, Isar reopened:<br><br>PUT **21:33:47.373** → restart **21:33:50.423** — **3.0 seconds**<br>PUT **21:35:06.744** → restart **21:35:11.432** — **4.7 seconds**<br><br>**The OS process never dies.** pid **5670** spans the entire window, before the first restart and after the second. This is activity/engine recreation, not a process kill.<br><br>**No crash record of any kind.** The crash buffer is empty; no ANR, no `lowmemorykiller`, no `am_kill`, no FATAL for the package in any buffer.<br><br>**The swipe-away does not explain it.** The project owner swiped the app from recents once, after both recordings. That accounts for at most one activity recreation. It does not account for two restarts three minutes apart — and **the first, at 21:33:50, preceded the swipe entirely**. Two further unprompted restarts followed at 21:50:53 and 21:51:22 with the phone untouched, making four in total before any deliberate force-stop.<br><br>**Memory was considered and is not supported by the evidence.** The app measured ~311 MB PSS with 202 MB swapped, on a device with 7.5 GB total and 2.3 GB available. `onTrimMemory` events in the buffer belong to other processes (Oplus UX, AIUnit) and are timestamped 21:39–21:41 — **after** both restarts, none attributed to uid 10639. Plausible, unproven, and recorded as unproven.<br><br>**MECHANISM UNDETERMINED.** What is established is the correlation: both restarts follow a large PUT (95 MB and 63 MB) within 3–5 seconds, and both abort it. What triggers the recreation is not known, and guessing further than that would be inventing a cause to close a row.<br><br>**Next step is instrumentation, not speculation** — activity lifecycle logging (`onDestroy`/`onTaskRemoved`/`onCreate`) would say directly whether the activity is being destroyed and by whom. That is a production change and belongs to its own decision round. <br><br>**CORRECTION, 2026-08-23 — the large-PUT correlation is REFUTED as a general rule.** Mission 8.2's scenario 3 recorded a single chunk of **612,597,902 bytes (613 MB)** and uploaded it end to end with **no engine restart at all** — `PUT` completed, metadata `201`, `PATCH` → `{status: complete, verified: true}`, `Chunk b8f372c3 uploaded.` at 13:46:29. That is **6.4x larger than the 95 MB chunk** this row cites as having triggered a restart.<br><br>**So size does not cause the restarts.** The apparent correlation was a confound: a larger upload occupies more wall-clock time and is therefore exposed to whatever interrupts it for longer. Every chunk whose PUT ran to completion has uploaded — 735 KB, 3.4 MB, 41 MB, 613 MB, 1.5 MB — at any size. The two that stuck are exactly the two that were interrupted.<br><br>**This narrows what the mechanism is NOT. It does not identify what it is, and the row's verdict stands: MECHANISM UNDETERMINED.** What can now be said is that a large PUT is neither sufficient nor necessary to provoke a restart, so an explanation resting on upload size or memory consumed by upload size is ruled out and should not be re-proposed without new evidence.<br><br>**It also strengthens [[item 137]] by elimination.** With size excluded, interruption is the only property the two stuck chunks share and the successful ones lack — which is precisely what that item describes. <br><br>**CLOSING NOTE, 2026-08-23.** The live reproduction was cleared on-device at the project owner's decision, by uninstall and reinstall after `pm clear` was refused by the device. This was test data, not preserved evidence. All findings and log evidence in this row stand unchanged; **the mechanism remains UNDETERMINED and is now unexaminable via this specimen.** A future reproduction would be needed, and the instrumentation this row asks for — activity lifecycle logging — remains the next step whether or not one occurs.<br><br>**CORRECTION, 2026-08-23 — this row's opening claim of causality is wrong as stated.** The row opens *"alongside [[item 137]], which it causes."* Ch 9.8 scenario 2's reproduction falsifies that as a **necessary** cause: item 137's symptom appeared in full — chunk orphaned at `uploading`, no retry after 2.5x Chapter 5.13's cap — with **zero engine restarts**.<br><br>**The signature is distinguishable, and the distinction is visible in the same log.** Genuine starts at 17:15:58, 17:55:06, 18:13:10 and 18:23:41 each carry `Vump Technologies 1.0.0+1 starting` **and** `ISAR CONNECT` immediately after their Impeller line. Bare Impeller lines carrying neither also occur, at 18:11:13 and 18:22:47. The only Impeller line after scenario 2's relaunch — pid **3965** at **18:23:44.576**, 3.4 s after that same pid's own start — is of the second kind: a renderer re-init within one launch, not a restart.<br><br>**A plain `am force-stop` mid-PUT is sufficient on its own.** This item is therefore one *way* to interrupt an upload, not the reason interruption orphans a chunk. The defensible relation is that both items share a consequence — an aborted PUT — and item 137 describes what happens afterwards, regardless of what did the aborting.<br><br>**Recorded as a correction rather than an edit to the opening sentence**, per this register's additive rule, so the reasoning that produced the original wording stays visible. **Everything else in this row is unaffected**: the four restarts, the absent crash records, the refuted size correlation and the UNDETERMINED verdict all stand. The instrumentation this row asks for is now *less* urgent to item 137, which no longer depends on it, and no less urgent to this item itself. | Mission 8.2, item 137 |
| 139 | **No usable credential can read Aurora or Secrets Manager from a developer terminal — and it is two separate failures, not one** | **Found at Mission 8.2 while trying to confirm [[item 137]]'s mechanism by reading the chunk rows directly. The read could not be performed by any available identity.**<br><br>**Cause 1 — `faisal-dev` lacks the IAM policy.** It authenticates cleanly (`sts get-caller-identity` returns `arn:aws:iam::929570731524:user/faisal-dev`) and is then refused by identity policy: **`AccessDenied` on `rds:DescribeDBClusters`** and **`AccessDeniedException` on `secretsmanager:ListSecrets`**, both reported as *"no identity-based policy allows"* the action. This is a missing grant, not a session problem — no amount of re-authentication changes it.<br><br>**Cause 2 — `vump-dev-operator` cannot be assumed here.** The role is MFA-gated per ADR-049 and the assume-role prompt has no stdin to read a code from in this non-interactive environment, so the call hangs until it is killed rather than failing. **A hang, not a denial** — which is why it was initially misread.<br><br>**The two are kept apart deliberately.** Mission 8.2's first hypothesis was *"blocked by the MFA gate"* and treated both identities as one problem. They are not: one identity is authenticated and under-privileged, the other is privileged and unreachable. **Fixing either alone would unblock the read; fixing the wrong one would not**, and a row that conflated them would send the next person to resync a clock — which is where this investigation actually went first, before establishing that `w32time` cannot even be started on this terminal and that drift was never involved.<br><br>**What it blocks.** Any device-verification work needing a direct read of Aurora or Secrets Manager. Concretely today: [[item 137]]'s mechanism stays UNCONFIRMED because the chunk rows' `status` column cannot be read, and the difference between *a row stuck at `uploading`* and *a claim query that never matched* is exactly what one `SELECT` would settle.<br><br>**Not an argument for widening access.** ADR-049 demoted human roles to MFA-gated and break-glass deliberately, and [[A-233]] records what a stray credential costs. The decision this needs is which of the two paths should exist for read-only diagnostics — a narrow read grant on `faisal-dev`, or a way to answer an MFA prompt from a terminal session — and that is a decision round, not a fix.<br><br>**Priority raised, 2026-08-23 — [[item 138]]'s causal link to [[item 137]] was refuted, so this credential gap is now the sole blocker on confirming 137's mechanism, currently the highest-severity open defect in the register.**<br><br>**CORRECTION, 2026-08-23 — this row no longer blocks [[item 137]].** The priority note above raised this row on the ground that it was *"the sole blocker on confirming 137's mechanism"*. **That ceased to be true within hours of it being written.** 137's mechanism was confirmed by reading the source — `claimNext`'s `queued`-only filter, the three in-process exits from `uploading`, and `requeue`'s `failed`-only guard — so the `SELECT` this row was blocking turned out not to be needed at all.<br><br>**The row stays open, unchanged, on its original merits.** `faisal-dev` is still refused by identity policy on `rds:DescribeDBClusters` and `secretsmanager:ListSecrets`, `vump-dev-operator` still cannot answer an MFA prompt from a non-interactive terminal, and both remain unfixed. What changes is only the justification for its priority: it blocks device-verification work that needs a direct read of Aurora or Secrets Manager **in general**, not item 137 **in particular**.<br><br>**Worth keeping as a lesson rather than just corrected.** The investigation spent real effort trying to buy a database read it did not need, while the answer sat in four files it already had permission to open.<br><br>**CORRECTION, 2026-08-24 — Cause 1 is wrong, and acting on it would have widened a user ADR-049 deliberately emptied.**<br><br>This row says *"`faisal-dev` lacks the IAM policy"* and names `rds:DescribeDBClusters` and `secretsmanager:ListSecrets` as missing grants. **They are missing on purpose.** `infrastructure/terraform/modules/iam/human.tf` states ADR-049 decision D-2 in its own words: *"The user holds no permission except `sts:AssumeRole`. … on its own it can read nothing, write nothing and describe nothing. Every working credential is a one-hour STS session obtained by assuming one of the two roles below, both of which require MFA. A disclosed key without the MFA device is inert."*<br><br>So the `AccessDenied` responses were **the design working**, not a gap. Granting those actions to the user would restore exactly the standing credential ADR-049 removed and [[A-233]] priced.<br><br>**There is one cause, not two.** Both halves of this row describe the same thing from different angles: an MFA-gated role assumption that a non-interactive terminal cannot complete. The `faisal-dev` calls failed because they were made **as the user** rather than **as an assumed role** — the correct way to call AWS here — and the operator call hung because nothing could answer the prompt.<br><br>**The permissions were never missing; they live on the role.** `vump-dev-human-operator` already grants, minimum-scoped: `rds-data:ExecuteStatement`, `BatchExecuteStatement`, `BeginTransaction`, `CommitTransaction` and `RollbackTransaction` on the cluster ARN alone; `secretsmanager:GetSecretValue` on the database credential secrets alone; and `secretsmanager:DescribeSecret` under `secret:vump/dev/*`. **That is precisely the set the migration runner uses** — it issues `BeginTransaction`, `ExecuteStatement`, `Commit`/`Rollback`, `DescribeSecret` and `DescribeDBClusters`. No Terraform change, no new policy and no console edit was ever required.<br><br>**The role name in this row is also wrong.** It says `vump-dev-operator`; `human.tf` builds `vump-${var.environment_slug}-human-operator` and the dev root module defaults that slug to `dev`, so it is **`vump-dev-human-operator`**. A reader following this row would have looked for a role that does not exist.<br><br>**And the tooling was already in place**, which nobody had checked. `~/.aws/config` already carries a `vump-dev-operator` profile with the correct `role_arn`, `source_profile = faisal-dev`, `mfa_serial = arn:aws:iam::929570731524:mfa/2_dev_faisal` and `region = ap-south-1`. The AWS CLI is installed (2.36.17). **The whole blocker reduces to a human typing six digits**, which the project owner can do and an automated session cannot.<br><br>**What stays true.** The clock investigation this row describes was still a dead end, `w32time` still cannot be started on that terminal, and drift was still never involved. **What changes** is that this row should never have been read as *"grant the user two permissions"*. It is *"assume the role interactively"*.<br><br>**No infrastructure fix is warranted for the MFA gate either.** It behaved correctly; what failed was a non-interactive process meeting an interactive prompt. A non-MFA role for scripted use would recreate the standing credential ADR-049 removed to save six keystrokes, and CI never needs one — ADR-049 gives automation GitHub OIDC.<br><br>**CORRECTION TO THE CORRECTION, 2026-08-24 — the second one was half right, and the wrong half is the half migrations need.**<br><br>The correction above says *"the permissions were never missing; they live on the role."* **True for `rds-data:*`, false for the credential those statements need.** Established by running the checks rather than reading the policy:<br><br>`sts:get-caller-identity` returned `assumed-role/vump-dev-human-operator/…` — the session works. `rds:DescribeDBClusters` returned `vump-dev-aurora`, **available**, Data API enabled — so that action was never the blocker either. `secretsmanager:ListSecrets` was **denied on the role too**, which confirms the original row's second named permission was never needed by anything; it is account-wide, cannot be resource-scoped, and only the diagnostic used it. Then a plain `SELECT current_database()` through the Data API **failed**: *"Failed to fetch secret `arn:…:secret:rds!cluster-d7c84f97-…` … not authorized to perform: `secretsmanager:GetSecretValue`."*<br><br>**Why.** `packages/migrate/src/cli.ts` derives its credential from `cluster.MasterUserSecret.SecretArn`, deliberately — migrations need DDL rights the seven per-function credentials do not have. The operator role's `GetSecretValue` is scoped to `values(var.db_credential_secret_arns)`, and the **RDS-managed master secret is not among them**. **`master_user_secret_arn` appears nowhere in `human.tf`**, so no human role can read it.<br><br>**And CI cannot either, on purpose.** `github-oidc.tf` carries an explicit `Deny` — `sid = "DenyMasterCredential"` — whose comment gives two reasons and rates the first higher: *"The master user bypasses every GRANT in migration 0007 and every trigger in 0006. A behavioural proof run as master would PASS WHILE PROVING NOTHING. Denying it is what keeps Gap 8's evidence real. It also keeps CI away from DDL, which is Gap 2's separate and undecided question."*<br><br>**So a plan to apply migrations through CI cannot work, and should not be routed around.** The Deny is a decision with a stated rationale, and Gap 2 is explicitly undecided.<br><br>**The correct path is `faisal-admin`, the break-glass administrator**, which ADR-049 retains for exactly this and which applied all nine prior migrations. Chosen for migration 0014 on 2026-08-24 because it needs no infrastructure change and does not pre-empt Gap 2. **The honest cost**: using break-glass for routine work erodes what break-glass means, and that is an argument for deciding Gap 2 rather than for widening a role quietly.<br><br>**The alternative was possible and was rejected as a decision, not as an obstacle.** The permissions boundary's `ReadEnvironmentSecrets` statement already lists `var.master_user_secret_arn`, so granting the operator role that secret is a policy edit rather than a boundary edit. It would hand a human role the credential that bypasses every GRANT and trigger — the thing the CI Deny exists to prevent — which makes it an ADR-049 posture change deserving its own ADR.<br><br>**This row has now been corrected three times.** Cause 1 was wrong, the role name was wrong, and the second correction overstated what the role can do. What has been true throughout is narrower than any version of it stated: **`vump-dev-human-operator` can read and query Aurora; it cannot run migrations.** | Mission 8.2, item 137, ADR-049 |
| 140 | **A session directory outlives a recording that produced no chunk** | **Noticed at Mission 8.2 while verifying Ch 9.8 scenario 2's on-disk state.** `app_flutter/recordings/` held **two** session directories: `1402ad24-bc52-40e2-9656-b889d9bd0512` with the expected 520,658,550-byte `0000.mp4`, and `1a1632d9-052d-40d0-9a68-8494c54085df` (18:13) **empty**, left behind by scenario 2's discarded first attempt.<br><br>**Small, and deliberately not folded into [[item 137]].** That item is about a chunk that exists and cannot be uploaded. This is the opposite — a directory with no chunk in it at all, so no queue row, no upload and no BR-08 question arises. **No data is at risk**; an empty directory costs an inode.<br><br>**What is NOT established is whether anything ever removes it.** One observation across one session shows a stray directory can be created, not that they accumulate. A device recording daily for a month would settle it, and no such run has been made. **Registered so the question is not lost, not because a leak is proven.** | Mission 8.2, item 137 |
| 141 | **Port fakes model the interface contract and not the concurrency, and a green suite proved nothing about the defect it was written for** | **Found at Mission 8.2 on 2026-08-23, by the fix it was supposed to protect failing on hardware.** [[ADR-052]] shipped with five tests, all passing, all reviewed. The change was broken on a CPH2707 the same day, and none of the five could have caught it.<br><br>**The mechanism.** `IsarChunkStore.watchQueue()` yields a first snapshot and only then subscribes to `watchLazy()`, which reports writes made after it attaches. A write landing in that gap produces no emission at all. `_FakeQueue` emits only when a test calls `push`, so it has **no first-read-then-attach gap for a write to be lost in** — the failure mode is not merely untested there, it is **unrepresentable**.<br><br>**The fake was faithful to the contract and silent about the timing**, and the timing was the entire defect. A test written against it asserts that the dispatcher reacts to an emission, never that an emission arrives.<br><br>**Sibling to [[item 136]], deliberately separate.** That item is *asserts a call happened, not what it carried*; this one is *asserts the contract, not the concurrency*. Same shape, different axis, and a different fix: 136 needs assertions on captured arguments, this needs fakes whose timing can be driven.<br><br>**Confirmed twice over while being written.** Making the dispatcher wake itself broke five existing tests, because `_FakeQueue.push` did not update the snapshot its reads returned — letting an emission and a later read disagree in a way the real store cannot produce. **The fake was wrong, not the fix.** So the blind spot was demonstrated a second time, inside the very change that recorded it.<br><br>**RECOMMENDS AN AUDIT; DOES NOT CLAIM ONE.** Only `_FakeQueue` has been examined. Every other fake standing in for a live Isar watch is *suspected* of the same shape and none has been checked. The question each should answer is narrow: **can a write that produces no notification be represented at all?** If not, no test over that fake can fail the way the device does.<br><br>**Not closable by unit tests alone.** Isar 3 needs a native core `flutter test` does not load, so no test can drive a real watch. The durable remedy is the one ADR-052 took after the fact — write code that does not depend on the timing, then assert the independence rather than the model. | ADR-052, item 136, Mission 8.2 |
| 142 | **An in-flight transfer does not migrate to a better network once one becomes available** | **Observed at Mission 8.2 on 2026-08-23, during Ch 9.8 scenario 4's low-signal field run.**<br><br>A 201,385,519-byte chunk was uploading over a degraded cellular link at roughly **35 kB/s** — one multipart part, in flight since 22:43:27.605. **Wi-Fi became the default network at 22:53:18.945** and passed validation 9 ms later. **42.6 seconds after that, the part — by then 9.9 minutes old — still had not finished.** It completed only after the app was restarted, whereupon the same chunk uploaded over Wi-Fi in **13 parts at about 2.5 s each**.<br><br>**NOT a crash, NOT data loss, and NOT a defect in the upload path.** An established TCP connection is bound to the interface it was opened on and does not move; this is how sockets work, and the transfer was making genuine progress the whole time. Nothing was lost, nothing was duplicated, and the chunk uploaded and verified. **Not fixed here, and no fix is proposed** — it is tracked because the field consequence is real, not because the behaviour is wrong.<br><br>**The consequence.** A Collector who walks from a dead zone into Wi-Fi range mid-upload gets **no benefit from the better link until the current part ends**. On a genuinely bad connection a single part can take ten minutes or more, and the app offers no way to hurry it. The remedy the project owner found by accident was to kill and reopen the app, which [[ADR-052]]'s reconciliation then recovered cleanly — an effective workaround that no part of the UI suggests and that a Collector has no reason to guess.<br><br>**LIMITATION, STATED RATHER THAN GLOSSED.** The observation window was **42.6 seconds**. That is enough to show the transfer did not *immediately* benefit from the better network; **it does not prove the part would never have completed**, nor that Dio or the OS would never eventually retry it onto the new interface. A longer wait was not run, because the project owner restarted the app first. Any fix proposal should begin by establishing that limit properly rather than treating this row as proof of a permanent stall. | Mission 8.2, ADR-052, Ch 9.8 |
| 143 | **A mission was closed on a verification sweep narrower than the one CI enforces** | **Found at Mission 8.2 on 2026-08-23, while preparing to merge Mission 8.1's branch — 38 commits after 8.1 was reported closed.**<br><br>`npm run typecheck` failed with **31 errors** across the four test files Mission 8.1 added to close the backend coverage exemptions. **CI runs `npm run typecheck` as a required step**, so the branch would have gone red on its first PR check. It had been red the entire time.<br><br>**The mechanism is that the two commands do not check the same thing.** 8.1 verified with `npm test`, and `vitest` runs through **esbuild, which strips types without checking them**. A test file can be thirty-one type errors deep and still report 322 passing. The gate that would have caught it was never run, and *"the full suite passes"* was true and beside the point.<br><br>**Structurally similar to [[item 141]], and deliberately not folded into it.** That item is about a *fake* that models an interface faithfully and its timing not at all. This is about a *verification sweep* that runs one real gate and not the other. Both produce a green signal that does not mean what it appears to mean, and the shared lesson is that **a check is only as good as its coverage of what actually gates the merge** — but the mechanisms differ, and so would the fixes: 141 needs fakes whose timing can be driven, this needs the closing sweep to match CI's job list.<br><br>**The fix surfaced a second gate on the way through.** Non-null assertions were the obvious repair and are forbidden here — `@typescript-eslint/no-non-null-assertion` is enabled, and 18 of them converted a red typecheck into a red lint. So the sweep is `typecheck` **and** `lint` **and** `test`, not any two of the three.<br><br>**Closed for this instance, open as a habit.** The 31 errors are fixed and every gate is green. What is not fixed is the general case: nothing mechanically ties a mission's closing checklist to `ci.yml`'s job list, so the next mission can drift the same way. **A CI-derived checklist, or a single local script that runs exactly what CI runs, would close it** — neither exists, and neither is proposed here. | Mission 8.1, Mission 8.2, item 141 |
| ~~144~~ | ~~**HIGH PRIORITY — the Pre-Recording Checklist has no way back: no AppBar button, no tabs, and system back backgrounds the app**~~ **CLOSED 2026-08-24 — FIXED AND DEVICE-VERIFIED on the CPH2707 that found it.**<br><br>**Fixed by two changes together, and neither alone would have done.** Task Detail now enters the checklist with `context.push` instead of `context.go`, which puts it on the root navigator **above** the shell — so it stays the full-screen modal Chapter 2.3 §5 intends, still with no tab bar, while leaving a page beneath it to pop back to. The checklist also gained an explicit `leading:` close action, falling back through `pop`, then Task Detail rebuilt from `selectedTaskProvider` (which holds the `projectId` the route does not carry), then the Collector root.<br><br>**The pairing is the point.** `push` alone leaves every non-push entry broken — a deep link, a restored route, or a `go('/checklist/…')` written later all produce a one-page stack again. An explicit close alone leaves the **back gesture** still backgrounding the app, because the stack shape is unchanged. **The device confirmed exactly this split**: the close button satisfied three checks and only `push` satisfied the fourth.<br><br>**PROVEN BY A TEST THAT FAILS AGAINST THE OLD CODE.** The router-level test asserts `router.canPop()` after entering the checklist through the real route table. Reverted to the `go` it replaced, it fails with **`Expected: true, Actual: <false>`** — the defect itself, not a proxy for it. It asserts the **stack shape rather than the button**, because the button is a consequence: `automaticallyImplyLeading` renders one exactly when the route can pop.<br><br>**Confirmed on device by the project owner, all four checks**: the close button is present; it returns to Task Detail; **the Android back gesture now returns rather than minimising**; and TalkBack announces it as *"Close"*.<br><br>**ONE CONSEQUENCE WORTH KNOWING, because it will surprise the next reader.** `push` appends an imperative match and deliberately leaves `currentConfiguration.uri` at the last **declarative** location — the same reason a push does not change the browser URL on web. So the URI now reads Task Detail while the checklist is the page on screen. An existing assertion broke on this and was **moved, not weakened**: a `topLocationOf` helper reads the top of the stack, the expected string is unchanged, and the test still asserts what it always meant — that the id in the route is the one tapped two screens earlier. Any future test asserting *which screen is open* must read the top of the stack, not the URI.<br><br>**Scope was and remains one screen.** Back works correctly on the Projects tab, Project Detail and Task Detail, which are nested inside the shell where `go` builds a parent stack. **[[item 145]] records the one place that shape might repeat and has not been checked.** | **Found on a CPH2707 at Mission 8.2 on 2026-08-23, during Ch 9.8 scenario 5's TalkBack sweep, and confirmed by the project owner's direct device testing.** Surfaced by an accessibility pass but **not an accessibility defect — it affects every user equally.**<br><br>**ROOT CAUSE.** `/checklist/:taskId` is declared **top-level** at `router.dart:559`, outside both `StatefulShellRoute`s, and is reached by `context.go('/checklist/$taskId')` from Task Detail. **`context.push()` appears zero times in this codebase**; all 19 navigations use `go()`, which replaces the stack rather than pushing onto it. A `go()` to a top-level route therefore leaves a single-page stack, and three things follow at once: **`canPop` is false**, so `AppBar`'s `automaticallyImplyLeading` renders no back button; **the bottom tab bar is absent**, because the route sits outside the shell; and **system back has nothing to pop**, so Android backgrounds the app instead of navigating.<br><br>**SCOPE CONFIRMED — THIS SCREEN AND NO OTHER.** The project owner tested the surrounding screens directly with TalkBack on: **the back button is present, working and correctly announced on the Projects tab, on Project Detail, and on Task Detail.** Those routes are nested inside the shell branch, so `go()` builds the parent stack and `canPop` is true — which is exactly why they behave and this one does not. **The checklist is the sole exception**, so this is a single-screen defect rather than a systemic navigation-pattern problem, and a fix does not need to revisit the route table wholesale.<br><br>**CONFIRMED AN OVERSIGHT, NOT A DELIBERATE BLOCK.** There is **no `PopScope` on this screen**. The codebase contains exactly two, `recording_screen.dart` and `local_processing_screen.dart`, both `canPop: false` and both documented as intentional — so the project knows how to refuse a pop on purpose and did not do it here. `RecordingGuard` is not responsible either: it gates `/recording/:id` for BR-04 and says in its own docs that *"a disabled button is not the gate"*. **The route's own comment calls this screen a *"full-screen modal from Task Detail"*** — a modal implies a dismiss affordance, and none was ever built.<br><br>**SEVERITY MULTIPLIER — the trap closes exactly when something is already wrong.** `pre_recording_checklist_screen.dart:104` gates the primary action: `onPressed: outcome.allPassed && !_starting ? _start : null`. **Start Recording is disabled until every check passes.** So a Collector who FAILS a check — permission denied, storage low, battery low, no network — has **no forward path and no in-app back path**, on the one screen where they most need to leave and fix something. The only other tappables are the per-row re-run `TextButton`s.<br><br>**WHAT IT IS NOT.** Not a crash, not a lockup, not data loss. **Verified on device: system back backgrounds the app** — it does not do nothing and it does not exit or crash — so the home screen, the task switcher and force-close all remain available. The failure is a **broken navigation contract for this screen**, not a trapped device.<br><br>**RECOVERY, PART VERIFIED AND PART DERIVED.** Backgrounding and resuming returns to this same screen, because the process is still alive — **verified**. A cold start after a force-close goes through the router's redirect chain to the Collector root instead — **derived from `router.dart`'s redirect, not tested on device**, and it should be checked before anyone relies on it.<br><br>**Not fixed here.** Mission 8.2 is a testing pass; this is reported, not repaired. The obvious repairs — `push()` instead of `go()`, an explicit `leading:` close action, or moving the route inside the shell — differ in what else they change about the route table, so the choice is a decision round rather than a patch. | Mission 8.2, Ch 9.8, Ch 2.3 §5, BR-04, item 145 |
| 145 | **SUSPECTED, NOT CONFIRMED — the Admin creation routes have the same shape [[item 144]] had** | **Raised at Mission 8.2 on 2026-08-24, immediately after 144 was fixed. This row is a question, not a finding.**<br><br>**What is actually known**, from reading the route table while tracing 144 and nothing more: `/admin/projects/new` and `/admin/projects/:projectId/tasks/new` are declared **top-level**, outside both `StatefulShellRoute`s, and every call site reaches them with **`context.go`**. That is the same three-part shape that made the checklist a dead end — a `go` to a top-level route leaves a one-page stack, so `canPop` is false, the AppBar renders no automatic back button, the tab bar goes with the shell, and system back has nothing to pop.<br><br>**One incidental observation, unverified and possibly irrelevant**: a scan of `AppBar(` counts across `lib/features/` showed `admin_create_project_screen.dart` with one and **`admin_create_task_screen.dart` with none at all**. A screen with no AppBar has no automatic back button to lose, which could make it worse or could mean it has some other affordance. **Neither was looked at.**<br><br>**WHAT HAS NOT BEEN DONE, stated so nobody reads this row as a defect.** No widget was inspected, no test was written, and **nothing was run on a device**. There may be an explicit `leading:`, a Cancel action, or a `PopScope` that makes the whole question moot — the codebase already contains two deliberate `canPop: false` screens, so a top-level route is not by itself evidence of anything.<br><br>**Why it is registered rather than investigated.** 144 was found by a person tapping a real screen, not by reading the router, and the same shape on the Admin side deserves the same standard of evidence rather than an inference from a grep. Mission 8.2 is a Collector-facing testing pass and never exercised the Admin surfaces at all.<br><br>**The trace this needs is small**: open both screens, check for a back or cancel affordance, and confirm what system back does. If it reproduces, 144's fix is the template — `push` for the stack shape and an explicit close for every entry a push cannot cover. | Mission 8.2, item 144 |
| 146 | **`develop` sat red for seven hours because the CI run nobody read had already failed** | **Found at Mission 8.2 on 2026-08-24, when PR #27's checks failed against content it had inherited rather than written.**<br><br>**What happened.** Mission 8.1's forty commits reached `develop` by **fast-forward, with no pull request**. `ci.yml` triggers on `push` to `develop`, so **CI ran and it failed** — run `32655952071`, `aa06faf`, 2026-08-23T17:47:12, jobs **Format** and **Backend**. The previous six pushes to `develop` had all passed. Nothing was broken about the check; **the result was simply never opened**.<br><br>**The verification that was reported instead** was a local sweep — `flutter analyze`, `flutter test`, `npm run typecheck`, `npm run lint`, `npm test` — run against the merged tree and reported as *"post-merge verification passed"*. Every one of those commands did pass. **None of them is the formatter**, and CI runs two: `dart format --set-exit-if-changed` over the tracked Dart files, and `npx prettier --check .` in `backend/`.<br><br>**The damage was small and that is not the point.** Two files, formatter-only, no logic: `test/features/upload/application/upload_dispatcher_test.dart` from `8cebcbc`, and `functions/redeem/src/federated_auth.test.ts` from `4b63164`. Fixed by running the two formatters. **The seven hours are the finding, not the diff.**<br><br>**THIS IS NOT [[item 143]]'S SHAPE, AND FOLDING THEM WOULD LOSE THE DISTINCTION.** 143 is a **missing** gate — a mission closed on `npm test` alone, where `vitest` goes through esbuild and never typechecks, so the failure was never observed by anything. **146 is an *unread* gate.** The check existed, ran on schedule, reached the correct verdict, and reported it on a page nobody opened. A better checklist fixes 143. Only *looking at the result* fixes 146, and no amount of local command-running substitutes for it.<br><br>**Surfaced by accident.** It became visible only because the next branch opened a PR, whose checks ran against the same inherited content and failed identically — same two jobs, same two files. Had Mission 8.2 also merged by fast-forward, `develop` would still be red and nobody would know.<br><br>**The standing correction**: after **any** merge or push to `develop`, fast-forward included, read the run the push triggers — `gh run list --branch develop` — rather than re-running local commands and calling that verification. | Mission 8.1, Mission 8.2, item 143, PR #27 |
| 147 | **Ch 9.4 target 5 has not been re-verified with the live preview running** | **Raised at Mission 8.2's follow-on on 2026-08-24, when ADR-053 put a live camera preview on the Recording Screen for the first time.**<br><br>Target 5 — *battery under 15% per 10-minute chunk* — passed on scenario 3's 25-minute run at **−6% and +4.3 °C over 24 min 7 s**, measured while the Recording Screen was a **black `ColoredBox`**. That screen now composites a camera texture for the whole session, so the figure it passed on no longer describes the software.<br><br>**A 5-minute sanity check was run and is not a substitute.** Unplugged, it measured **−2% over 8 min 7 s** against a pro-rated **−2.0%**, and **+0.4 °C** against a pro-rated **+0.96 °C**, with no crash, no engine restart, and a 309 MB chunk recorded and uploaded normally. The project owner described the device as *"slightly heating, not much"*, which agrees with the measured rise.<br><br>**Why that is a gate and not a result.** Thermal rise is not linear — the first minutes of a run are the flattest — and the device started at 37.1 °C already warm from charging, so it began nearer equilibrium than a cold start would. Five minutes cannot resolve a slow climb, and whole-percent battery resolution puts ±1% around every reading at that scale. It can catch an obvious regression and did not find one.<br><br>**What is owed**: scenario 3's full 25-minute run repeated with the preview active, compared against −6% and +4.3 °C. **Ask the heat question while the device is still hot**, which is the cheap fix scenario 3 identified after recording that sub-check as UNASSESSED.<br><br>**It also raises [[item 143]]'s sibling concern about target 4.** *60 fps sustained, zero jank* is deferred to Mission 8.3 for want of a profile build, and a preview compositing every frame is exactly the workload that target measures — so it becomes more important, not less. | ADR-053, Ch 9.4, Ch 9.8, Mission 8.3 |
| 148 | **TEMPORARY — `tasks.shared_with_org` relaxes BR-19 so any Collector in the org can see one Task** | **Added at 2026-08-24 by migration 0014. This is a scoped, reversible exception, not a feature. [[item 89]] and [[item 92]] — real Task assignment — supersede it, and it is meant to be deleted, not built on.**<br><br>**Why.** BR-19 scopes a Collector to Projects with an assigned Task, enforced in **three** places, not two: `GET /v1/projects`'s Collector branch, `GET /v1/projects/{id}/tasks`'s Collector branch, and `assertProjectVisible`, which gates the second. The consequence is correct and unusable for a new tester: a Collector with no `task_assignments` row sees an **empty Projects list, no error, nothing to act on**. The only route that creates an assignment, `POST /v1/tasks/{taskId}/assignments`, is Admin-only, and **no Admin account exists** — so there was no path out from inside the product.<br><br>**EXACTLY WHAT TO DELETE when 89/92 ship**, so nobody has to re-derive it:<br>1. the column `tasks.shared_with_org` (migration **0014** added it, migration **0015** moved which row carries it; forward-only, so removal is a new migration)<br>2. the `OR t.shared_with_org` clause in **all three** queries above, restoring each `LEFT JOIN task_assignments` to an inner `JOIN` with the predicates back in the `WHERE`<br>3. the seven tests tagged `— 0014` in `functions/projects/src/index.test.ts` and `functions/tasks/src/index.test.ts`<br>4. **this row**<br><br>**Migration 0015 is part of the same mechanism, not a separate thing.** 0014 flagged the task this project had recorded against, whose title is *"F2 pagination seed 68"* — one of the 201 pagination fixtures in a 202-task project. A tester opening the app saw that as their only assignment, which reads as a bug rather than as work. 0015 moves the flag to `208f416c-a707-4df4-a25c-5b45707803b0`, **"Checkpoint walkthrough"** — *"Record a short walkthrough"* — the one non-fixture task and the oldest in the project. **It is a migration rather than an `UPDATE` against dev** because 0014's checksum is recorded and editing it is a hard error under ADR-046; a direct update would have fixed dev and left every other environment running the wrong id.<br><br>**What it deliberately does NOT relax, and this is the part to check if it is ever edited.** **Every one of the three queries keeps `p.org_id = :orgId`**, so a Task flagged in one organisation stays invisible to every other — BR-20's isolation is untouched. Admin branches never joined `task_assignments` and are unchanged. `ta.removed_at IS NULL` survives, so Chapter 4.8 §3's *"immediately excludes that Task from all future queries"* still holds for a withdrawn assignment.<br><br>**One implementation detail worth its own line, because getting it wrong fails silently.** The assignment predicates moved **into the `ON` clause**. Left in the `WHERE`, `ta.user_id = :userId` would discard exactly the rows the `LEFT JOIN` produced a NULL for — the shared ones — and the relaxation would compile, read correctly and do nothing. A test pins the placement.<br><br>**No ADR, and the reasoning was confirmed after implementing rather than assumed before.** Nothing about the data model's meaning changed and neither did the authorization pattern: a Collector still sees a Task because a row says they may, and this adds one more kind of row that can say so. An ADR records a change of pattern; this is an exception to a rule, which is what this register is for. **That judgement would have flipped under the rejected alternative** — auto-assigning at provisioning would have widened `vump_auth_verify`'s grants beyond `SELECT, INSERT ON users`, and a change to the posture of the one role that touches `users` would have needed its own ADR.<br><br>**Environment reality.** The mechanism is in every environment; **which row carries the flag is not**, because seeded data is not. Migration 0014's `UPDATE` names the Checkpoint Project task this project has recorded against throughout Missions 7 and 8 and is a **no-op wherever that row does not exist**. Any environment wanting a shared Task sets the flag on one of its own rows. **Nothing here is dev-only.**<br><br>**WIDENED 2026-08-25 — the flag now sits on EIGHT tasks, not one. Migration 0016.**<br><br>This row's *"touches exactly one row"* and *"names the Checkpoint Project task this project has recorded against"* described migration 0014's state and then 0015's. **Both are now out of date on the count**, and the count is the thing a reader would use to check whether the relaxation had grown while nobody was looking — so it is corrected here rather than left to be inferred.<br><br>**Why.** 0015 left the flag on one task, so a newly-invited Collector saw exactly one job. Mission 8.2A renamed the project to **Tailoring Workshop** and eight of its tasks to real garment-workshop assignments; sharing one of the eight would have produced a workshop with a single visible job in it. Ruled at Part 5.<br><br>**WHAT DID NOT CHANGE, which is the whole point of recording it this way.** Same column, same `OR t.shared_with_org` in the same **three** queries, same supersession by [[item 89]] and [[item 92]], same deletion list above, same temporary status. **Only the number of flagged rows moved.** Nothing about the mechanism, its blast radius or its lifetime is different at eight than at one.<br><br>**BR-20 is still untouched, and this is worth restating every time the count grows.** All three queries keep `p.org_id = :orgId`, so eight flagged tasks in this organisation remain invisible to every other. **Widening the count does not widen the blast radius** — the boundary that matters is the org check, not the number of rows inside it.<br><br>**Verified after applying, not assumed**: `SELECT count(*) FROM tasks WHERE shared_with_org` returned **8** across the whole database, against **1** immediately before, and none of the eight still carries a seed name.<br><br>**When 89/92 ship, the deletion list is unchanged** — dropping the column removes all eight at once. There is no per-row cleanup to remember.<br><br>**CORRECTION, 2026-08-25 — "three queries" was wrong. It is SIX gated sites, and the three this row missed are the three that WRITE.**<br><br>**Found by a real upload failing on a real device.** A Collector holding no `task_assignments` row at all recorded five chunks against shared tasks; every one was refused with `404 RESOURCE_NOT_FOUND — That task was not found.` The footage was fine on disk and could never upload.<br><br>**What this row said**, and what migration 0014's header repeated: *"the `OR t.shared_with_org` clause in all THREE queries"*. Those three are the ones that **list** things — `GET /v1/projects`'s Collector branch, `GET /v1/projects/{id}/tasks`'s Collector branch, and `assertProjectVisible`. **Every query that WRITES was left demanding a hard assignment.** A `grep` for `task_assignments` across `functions/*/src/index.ts` finds six gated sites, not three:<br><br>READ, relaxed when this row was written: `projects` Collector branch; `tasks` `listTasks`; `tasks` `assertProjectVisible`.<br>**WRITE, missed entirely and relaxed on 2026-08-25:** `sessions` `assertAssigned`; `chunks-upload` `resolveSession`; `chunks-verify` `resolveChunk` — the last of these **strengthened as well as widened**.<br><br>**The consequence was worse than an incomplete feature.** The read half let a Collector see a shared task, open it, pass the Checklist and record; the write half then refused the session. **The product told them to do something and then discarded the result** — and only BR-08 keeping the file until upload is confirmed stopped it being data loss.<br><br>**The two chunk functions ask a different question, and only one clause moved.** Their assignment join is on `ta.user_id = s.collector_id` — *the session's owner* — while `s.collector_id = :callerId` is the ownership check. **Only the first was relaxed.** Widening what the session owner may reach says nothing about who may call.<br><br>**`chunks-verify` is strengthened by the same change.** That query never joined `projects` at all, so BR-20 rested transitively on `s.collector_id = :callerId`. Converting the assignment join to a `LEFT JOIN` would have made caller identity the **only** org defence there, so `JOIN projects p` and `p.org_id = :orgId` were added. **This site now asserts BR-20 where it previously inferred it.**<br><br>**THE DELETION LIST ABOVE IS CORRECTED ACCORDINGLY.** When [[item 89]] and [[item 92]] ship, the `OR t.shared_with_org` clause must be removed from **six** queries, not three, and each `LEFT JOIN task_assignments` restored to an inner `JOIN` with its predicates back in the `WHERE`. **`chunks-verify`'s new `p.org_id = :orgId` and its `projects` join must NOT be removed** — they are a correction that outlives the relaxation.<br><br>**Why the miss happened, stated so the pattern is visible.** The trace followed the question *"where is a Collector's task list scoped?"* and answered it correctly. The question that mattered was *"where is a task's reachability enforced?"*, and the write path enforces it three more times. **A relaxation traced through read paths only is a half-relaxation, and a half-relaxation on a write path fails after the user has already done the work.** | Migrations 0014, 0015, 0016, BR-19, BR-20, Ch 4.8 §3, item 89, item 92, Mission 8.2A |
| 149 | **The `default` AWS profile is `faisal-admin`, so every command without `--profile` runs as unscoped break-glass** | **Found at Mission 8.2's follow-on on 2026-08-24, while looking for a credential that could apply migration 0014.**<br><br>`aws sts get-caller-identity --profile default` returns `arn:aws:iam::929570731524:user/faisal-admin`. `~/.aws/credentials` holds `[default]` and `[faisal-dev]`, and the two MFA-gated role profiles in `~/.aws/config` both source `faisal-dev` — so the scoped path is configured correctly and **is not the one a bare command takes**.<br><br>**This quietly undoes what ADR-049 set up.** That ADR made `faisal-dev` the day-to-day identity holding nothing but `sts:AssumeRole`, and kept `faisal-admin` as break-glass — *"Gap 1 closes to `faisal-admin` is break-glass, never to `faisal-admin` is gone"*. A default that resolves to the administrator inverts the intent: the safe identity requires a flag and the unscoped one is what you get by forgetting it.<br><br>**Not a vulnerability, and worth saying so.** The key is on one developer's machine, it was already there, and nothing is more exposed today than yesterday. What changed is that it is now **known** rather than assumed, and every AWS command in this session had to name `--profile` explicitly to avoid it.<br><br>**The fix is a local configuration change and was deliberately not made.** Swapping `[default]` to the `faisal-dev` key and giving the administrator its own named profile — `[faisal-admin]` — costs nothing and restores the intent. It is the project owner's machine and their credentials file; changing it unasked would be the wrong kind of helpful.<br><br>**One caveat for whoever does it.** Some tooling assumes a usable `default`, and `~/.aws/config`'s two role profiles name `source_profile = faisal-dev` explicitly, so they keep working either way. The migration runner uses the ambient credential chain with no profile argument at all, which is precisely why it reached `faisal-admin` and precisely why this matters. | ADR-049, item 139, Mission 8.2 |
| 150 | **Nothing deploys this project. Merging to `develop` changes the repository and stops there** | **Found at Mission 8.2's follow-on on 2026-08-24, by a merged fix that was live nowhere.** PR #30 merged at 12:00, `develop`'s CI went green, migrations 0014 and 0015 were applied and verified — and the tester still saw an empty Projects list, because `vump-dev-projects` and `vump-dev-tasks` were still running code built on **19 August**.<br><br>**The repository has two workflows, `ci.yml` and `pr-title.yml`, and every job in them is a check.** `format`, `analyze`, `test`, `android-build`, `codegen-drift`, `architecture`, `goldens`, `environment-consistency`, `secret-scan`, `aws-credential-isolation`, `workflow-trigger-guard`, `terraform`, `backend`. **There is no deploy job.** `backend` bundles eight functions into `artifacts/` and nothing ships them.<br><br>**The deploy is real, documented and manual**: `npm run build` in `backend/`, then `terraform plan` and a human `terraform apply` from `environments/dev`. `infrastructure/terraform/README.md` states the constraint and its reason — *"`terraform apply` is run deliberately, by a human, never by CI … A pipeline that can apply infrastructure is a pipeline that can destroy it"* — and `main.tf` records the build coupling deliberately, since *"Terraform is not a build tool."* **Neither document presents applying as a step that follows a merge**, and nothing prompts for it.<br><br>**The evidence that it is forgotten rather than deferred** is in the deploy timestamps themselves: 19 August for five functions, 20 August for two, and nothing since — across five days in which backend code merged. Whatever shipped on those dates shipped because somebody remembered.<br><br>**What it cost here.** Three people believed the fix was live: the change was merged, CI was green, the schema was applied, and the APK was re-sent to the tester on the theory that the app was stale. **None of those checks could have detected it**, because each was true. The one question nobody asked was whether anything had deployed.<br><br>**NO SOLUTION IS PROPOSED HERE, deliberately.** The obvious answer — deploy on merge — is the one the README already refuses, with a stated reason that is about blast radius rather than convenience. Any future mission has to work **within** that constraint, not remove it: a deploy that is prompted, recorded and visible without being automatic. **That is a design question, and naming the gap accurately is this row's whole job.**<br><br>**Interim mitigation, written down because it is the only thing standing in for a pipeline**: after merging backend code to `develop`, check `aws lambda list-functions --query "Functions[?starts_with(FunctionName,'vump-dev-')].{n:FunctionName,m:LastModified}"` and compare against the merge time. | Mission 8.2, ADR-046, item 146 |
| 151 | **Chunk metadata is not co-located with the footage, and is not in S3 at all — the bucket cannot be read as a dataset without the database** | **Found at Mission 8.2A's Part 1 trace on 2026-08-25, while researching how egocentric datasets are conventionally stored.** `POST /v1/chunks/{id}/metadata` writes to Aurora's `chunk_metadata` table (`functions/metadata/src/index.ts`). **Nothing writes a metadata object to S3.** So `s3://vump-platform-dev/{org}/{project}/{task}/{session}/0003_{chunk}.mp4` is footage with no accompanying context: reconstructing what a chunk *is* requires database credentials, a running Lambda, and knowledge of an eighteen-column schema.<br><br>**Why this is a finding rather than a preference.** Every comparable egocentric dataset co-locates. Project Aria's AEA format states the convention directly — *"each recording is stored as its own sequence with all data related to that recording self contained within that sequence folder"* — pairing `recording.vrs` with a sibling `metadata.json`. Ego4D ships a master `ego4d.json` plus a per-dataset `manifest.csv`; EPIC-KITCHENS-100 partitions by participant with composite ids. For a company whose product **is** the dataset, a bucket that cannot be interpreted without a live database is a blob store rather than a dataset. There is also **no session-level manifest**, so a truncated session is indistinguishable from a complete one by inspection — nothing in S3 records how many chunks a session should contain.<br><br>**THE S3 KEY HIERARCHY ITSELF IS NOT THE PROBLEM AND MUST NOT BE 'FIXED'.** Volume 5 Chapter 5.14 §1's `{org_id}/{project_id}/{task_id}/{session_id}/{sequence_index:04d}_{chunk_id}.mp4` already **is** the org/project/task/session/chunk hierarchy, is deterministic, is unique without coordination, and is what BR-11's *"never a duplicate object in S3"* guarantee rests on. It is better organised than Ego4D's flat UUID layout. The Mission 8.2A ruling confirmed it stands unchanged.<br><br>**WHY THIS NEEDS AN ADR AND CANNOT BE IMPLEMENTED AS A SMALL FIX.** The obvious remedy — a sidecar `…/0003_{chunk_id}.json` beside each chunk — **contradicts ADR-011, which is Accepted, in two separate clauses**:<br>1. *"Chunk buckets hold chunks and nothing else"* — because keys begin at `{org_id}` with no common prefix, **every lifecycle rule on these buckets is necessarily bucket-wide**, so a second object class inherits raw footage's policy. Under ADR-012 a small JSON would age into Glacier alongside a 700 MB video, which is precisely backwards for the file most likely to be read.<br>2. A scoping prefix was **already considered and rejected** — *"it contradicts the authoritative key in Volume 5.14, would invalidate the deterministic-key guarantee that BR-11 rests on, and orphans any object already written."*<br><br>Per CLAUDE.md's governance rule, changing an Accepted ADR means stopping and proposing a new one. **This row is that stop.** ADR-011 also reserves derived-data storage *"for future architecture"* while recording the bucket-wide-lifecycle constraint that shapes it; this is the first concrete consumer of that reservation.<br><br>**Deliberately not designed here.** The Mission 8.2A ruling treats it as separable future work, and no ADR is drafted. Naming the gap accurately is this row's whole job — the same restraint ADR-011 itself applied when it reserved the question rather than answering it early.<br><br>**No backfill is implied, under any option.** Metadata for every chunk already uploaded is intact in Aurora and can be exported whenever the layout is decided. **Nothing already in S3 moves or is renamed**, which is the property that keeps BR-11 closed. | ADR-011, ADR-012, Ch 5.14 §1, BR-11, Mission 8.2A |
| 152 | **The recording pipeline cannot observe itself — neither the encoder it used nor the frames it dropped is knowable in process** | **Deferred at Mission 8.2A Part 4 on 2026-08-25, after `capture.encoder_type` was scoped into that mission's metadata work and then removed from it.** It was removed because it is not a metadata field that happens to be missing; **there is no accessor to add**.<br><br>**The project already knew, and said so.** A-058, quoted verbatim in `camera_recording_pipeline.dart`: *"'Never a software encoder' is unverifiable, and may not always hold. CameraX selects the encoder; it can fall back to software on some devices and configurations, and nothing in the Dart API reports which was used. For a dataset business this is a silent quality risk, not a cosmetic one."* The same amendment records a second, related blindness: *"the flush interval is unobservable and unconfigurable"*, so Chapter 5.4 §1's promise that a crash loses *"at most the last buffer interval"* holds in shape but not in size.<br><br>**Two things sit behind one cause**, which is why they share a row rather than getting one each:<br>1. **Encoder identity** — hardware or software. Reaching `MediaCodecInfo.isHardwareAccelerated()` (API 29+) means a platform channel into CameraX's internals, not a Dart call.<br>2. **Frame-timing and drop instrumentation** — encoded / dropped / expected frame counts, queue depth, backpressure, target-versus-actual bitrate. **This project has no internal performance instrumentation of any kind.** Every performance figure on record — Chapter 9.4's targets, Mission 8.2 scenario 3's thermal climb from 3 to 12 — was measured through `adb` from outside the app, by a human watching. Nothing the app ships can report on itself.<br><br>**Why it is worth doing eventually, and why not now.** A dataset whose quality varies silently by device is the specific risk A-058 names, and no amount of manual device testing scales to a Collector fleet. But it is native work behind a channel, not a field addition, and Mission 8.2A's ruling kept that mission to what was genuinely reachable. **The precedent for the mechanism already exists** — `DeviceModelChannel.kt` and `FreeSpaceChannel.kt` are two working platform channels with Dart wrappers, so the pattern is established rather than novel.<br><br>**Do not close this by adding a nullable column.** A field that is always null, or filled with a guess, is worse than an absent one: it makes an unmeasured property look measured, which is the failure mode `MetadataCaptureConditions` was deliberately designed to avoid — *"a plausible-looking wrong value is harder to catch than an obvious empty one."* | A-058, Ch 5.4 §1, Ch 9.4, Mission 8.2A, item 147 |
| 153 | **IMU / gyroscope capture is the most valuable egocentric metadata this project could add, and the relational schema cannot hold it** | **Deferred at Mission 8.2A Part 4 on 2026-08-25.** Separated from [[item 152]] deliberately: that row is about a pipeline that cannot see itself, **this one is a data-capture feature blocked on a storage question**, and conflating them would hide the fact that the blocker is different.<br><br>**Why it is the valuable one.** For egocentric footage, head and body motion is signal rather than context — walking cadence, stability, where attention turned and when. It is the field the reference metadata this mission was shown spends most of its structure on, and the one thing on that list with a real Android equivalent (`sensors_plus`, or the platform's own sensor API) rather than an ARKit-only origin.<br><br>**Why it does not fit.** Every other metadata field is a scalar, and `chunk_metadata` is eighteen fixed columns (`0004_sessions_chunks_metadata.sql`). **IMU is a time series** — hundreds of samples per second for up to ten minutes per chunk. It does not belong in a column, and it does not belong in a `jsonb` blob either at that volume. It belongs beside the footage, which is precisely the layout question [[item 151]] raises and deliberately does not answer.<br><br>**So the dependency runs one way and should not be inverted:** item 151's ADR decides where per-chunk data that is not a scalar lives; this row is its first real consumer. Implementing IMU capture first would force that architectural decision under feature pressure, which is the failure ADR-011 avoided when it reserved derived-data storage rather than inventing it early.<br><br>**A new dependency is also required** and must go through ADR-030's six-item admission checklist — `sensors_plus` is not in `pubspec.yaml` and neither is any sensor package.<br><br>**Not scoped, not estimated, and not promised.** Whether this project wants AR-grade motion data at all is a product question that the garment-workshop use case it is currently being tested against does not answer. Recording that the option exists, and what stands in front of it, is this row's whole job. | item 151, ADR-011, ADR-030, Ch 4.5, Mission 8.2A |
| 154 | **The Checklist's battery floor is 5 %, not the 20 % three volume passages state** | **Changed on 2026-08-25 by the project owner, during Mission 8.2A's device verification.** A CPH2707 at 17 % could not start a recording, and the threshold that blocked it is a product setting rather than a law.<br><br>**This does not contradict a requirement, and the distinction is the whole reason it is a one-line change.** FR-CHK-03 names a *"configured minimum threshold"* and **gives no number**. The 20 came from three worked examples that agree with each other: Volume 1's UC-05 remedy — *"Charge your device to at least 20% before recording"* — Volume 2 Chapter 2.7's C-08, and Chapter 2.9 §3. `ChecklistOutcome.minimumBatteryPercent` called that *"a specification in everything but placement"*, which was a fair reading and is why this row exists: **the code now disagrees with all three, deliberately, and that disagreement is tracked rather than latent.**<br><br>**What it costs, stated rather than buried.** A 10-minute 1080p encode started at 6 % will not finish on most handsets. BR-08 keeps the chunk file until its upload is confirmed, and Chapter 5.3's crash recovery either completes a partial chunk or discards it, so the failure is bounded — **but bounded still means a Collector can lose the tail of a session that 20 % would have stopped them beginning.** The old floor was not arbitrary; it was buying that. A future mission may want a middle value, or a warning that is not a gate.<br><br>**Why nothing else needed editing.** The Checklist's remedy copy interpolates the constant — *"Charge to at least N% before recording"* — so the user-facing string followed automatically, and every test but one asserts against the symbol rather than the number.<br><br>**Two tests moved, and the second one is the interesting one.** The first pins the value, so a silent drift back to 20 fails loudly. The second used a literal `batteryPercent: 5` to mean *"a failing battery"* — and the moment the threshold became 5 that literal **silently became a passing one**, which is how the change was caught. It now reads `minimumBatteryPercent - 1`, the convention the same file already used two tests earlier. **A fixture written as a literal where a relative value was meant is a defect waiting for exactly this kind of edit.** | FR-CHK-03, BR-04, V1 UC-05, V2 Ch 2.7 C-08, V2 Ch 2.9 §3, Mission 8.2A |
| 155 | **`vump_chunks_verify`'s missing `projects` join is deliberate design, not a gap — and 'strengthening' it broke every upload** | **Registered 2026-08-25 after the misdiagnosis was deployed and had to be reverted the same evening.**<br><br>**What was believed.** `chunks-verify`'s `resolveChunk` had no `p.org_id = :orgId` clause, so BR-20 rested transitively on `s.collector_id = :callerId`. That was read as an oversight worth correcting while the query was already being edited, and Mission 8.2A's Part 21 report described the site as *"strengthened as well as widened"*.<br><br>**What is true.** `vump_chunks_verify` cannot read the tables that check would need. Verified live rather than inferred from a migration:<br>`has_table_privilege('vump_chunks_verify','tasks','SELECT')` → **false**; `('projects','SELECT')` → **false**. It holds `chunks`, `chunk_metadata`, `sessions` and `task_assignments`, and nothing more.<br><br>**So the absence was a consequence of the role's minimal grant, not a forgotten clause.** The original design was coherent: a role that may transition a chunk's status and touch almost nothing else, with org isolation carried by caller identity because a caller reaches only their own sessions and a session belongs to one org.<br><br>**What it cost.** The added `JOIN tasks` / `JOIN projects` failed with `42501 permission denied`, unhandled, surfacing as **`500` on every `PATCH /v1/chunks/{id}/status`**. Uploads reached S3 and posted metadata successfully and then could never be marked complete, so every chunk cycled `uploading → queued` forever. **The fix for one defect became a worse defect on the same route**, and it was live for roughly three hours.<br><br>**Why no test caught it.** Every backend test mocks the Data API, so a query the role may not execute passes exactly as one it may. **A mock asserts what was SENT and never what Postgres would ACCEPT** — the same gap A-212 named when a missing type hint reached a device. `chunks-verify`'s tests now assert the query touches **only** `chunks` and `sessions`, which is the property a mock CAN check.<br><br>**DO NOT RE-ATTEMPT THIS UNDER A BUG FIX.** If org isolation at this site is ever judged to need stating explicitly, it requires granting `SELECT ON tasks, projects` to a role deliberately kept minimal — a change to the posture ADR-015's domain separation establishes, and therefore its own decision with its own record. It is not a line to add while fixing something else. | ADR-015, BR-20, migrations 0007/0010, item 148, item 156, A-212, Mission 8.2A |
| 156 | **A revoked assignment no longer stops a Collector completing chunks in a session they already own** | **Registered 2026-08-25 as the accepted cost of item 155's revert.**<br><br>`chunks-verify`'s `resolveChunk` previously joined `task_assignments` and required a live row. That join is gone: it could not be made shared-task aware, because `shared_with_org` lives on `tasks` and this role cannot read `tasks` — so an unrelaxed join refused **every** chunk of a shared task, and a relaxed one could not be written. [[item 155]] carries the full reasoning.<br><br>**What narrows.** Chapter 4.8 §3 says a removed assignment *"immediately excludes that Task from all future queries"*. **A Collector whose assignment is revoked mid-session can now still transition the status of chunks belonging to a session they already own.**<br><br>**Why it was judged acceptable rather than worked around.** The reach is narrow and every boundary that matters still holds: `s.collector_id = :callerId` means they touch only their own sessions; the session was created while they were legitimately entitled, because `sessions/assertAssigned` gates that and **can** see `tasks`; the bytes are already in S3; and the transition available is a status change on a chunk they recorded. **Legitimacy is established once, at session creation, rather than re-asked at every step.**<br><br>**What would restore §3's literal reading.** Either granting this role `SELECT ON tasks` — see item 155 on why that is its own decision — or moving the entitlement check to something the role can see, such as a session-level flag written at creation. **Neither is attempted here**, and this row exists so the narrowing is a recorded decision rather than a silent one.<br><br>**This becomes moot when [[item 89]] and [[item 92]] ship.** Real Task assignment removes the shared-task mechanism entirely, at which point the join can be restored unconditionally — and the deletion list in [[item 148]] is where that is tracked. | Ch 4.8 §3, BR-19, item 148, item 155, item 89, item 92, Mission 8.2A |
| 157 | **`PreviewFrame.quarterTurns` reads a value that is often STALE, and that is correct — do not 'fix' it** | **Written 2026-08-25 to stop a change that has now been proposed twice and would break the preview both times.**<br><br>**What a reader sees.** `_frameOf` derives `quarterTurns` from `value.deviceOrientation`, and that value updates **only when the accelerometer fires** — `DeviceOrientationManager.start()` registers an `OrientationEventListener` and nothing else, so a window rotated by `SystemChrome` on a handset lying still refreshes it not at all. Measured on a CPH2707: `win=2378x1080` landscape, `deviceOrientation=portraitUp`, `quarterTurns=0`. **It looks obviously wrong.**<br><br>**Why it is right.** The plugin's preview delegate subtracts the same quantity before the widget adds it:<br>`rotationCorrection = currentDefaultDisplayRotation - preappliedRotationQuarterTurns` — `surface_texture_rotated_preview.dart:107`, with `image_reader_rotated_preview.dart:117` doing the same in degrees.<br><br>`preapplied` comes from `getPreAppliedQuarterTurnsRotationFromDeviceOrientation`, using **the same table** this project uses, fed by **the same platform stream**: `camera_controller.dart:338` and `android_camera_camerax.dart:1006` are two subscribers to one `onDeviceOrientationChanged()`. So:<br><br>`net = (displayRotation − preapplied) + ourQuarterTurns = displayRotation`<br><br>**The two stale terms cancel.** Staleness is invisible in the result by construction, and the ONLY term that ever mattered is `displayRotation` — which the plugin seeds at camera creation (`android_camera_camerax.dart:433`). **[[item 155]]'s sibling problem, and ADR-054's third amendment fixed it** by moving the landscape lock ahead of `openSession`, so the seed is correct before the camera exists. On a landscape-only route the window cannot change again without a physical turn, which fires the sensor and refreshes every term together.<br><br>**What breaking it looks like.** Source `quarterTurns` from `MediaQuery`, or from the locked capture orientation, or from anything window-derived, and the delegate still subtracts the `deviceOrientation` term. The cancellation fails and the preview lands **a quarter turn out**. Sourcing it from `lockedCaptureOrientation` is worse still: ADR-054 locks that to a constant, so the preview would freeze — the exact failure decision 2 exists to prevent.<br><br>**This is the third time on this code path that 'make the stale value fresher' has been the wrong instinct**, and the pattern is not staleness. It is **touching one side of a compensating pair**: ADR-053's fourth amendment (aspect ratio, which genuinely did belong to the window), ADR-054 §3 (the same arithmetic, reasoned correctly), and [[item 155]] (`chunks-verify`'s org join, where the same instinct broke every upload for three hours).<br><br>**ENFORCED, not merely documented.** `camera_recording_pipeline_test.dart` pins all four orientations against the plugin's own table, asserts the locked capture orientation does NOT move it, and asserts a stale `portraitUp` is reported faithfully as `0`. **A future 'fix' fails in CI rather than on a handset** — which is the whole point, because every previous version of this mistake was found by a person looking at a screen.<br><br>**What WOULD justify revisiting.** A plugin upgrade that changes or removes the pre-applied subtraction. The three tests are pinned to the plugin's arithmetic, so they are the thing that should be re-read against `surface_texture_rotated_preview.dart` after any `camera_android_camerax` bump. | ADR-053, ADR-054 §3, item 155, camera_android_camerax 0.7.4+5, Mission 8.2A |
| 158 | **Chapter 5.1 §1 says the preview is "full-bleed". It is not, deliberately — it is framed the way the handset's own camera app frames its own** | **Decided 2026-08-26 by the project owner, looking at the two side by side on a CPH2707.**<br><br>**What the chapter says**, verbatim: *"Render the live preview shown **full-bleed** on the Recording Screen (C-09, Chapter 2.7)."* Mission 8.2A's Part 1 trace flagged that the implementation letterboxed instead, and recorded it as an open gap against the chapter.<br><br>**Closing that gap literally was tried, and rejected on sight.** `BoxFit.cover` does fill every edge — by scaling until both axes are covered and clipping the overflow. On a 2.2:1 screen showing a 16:9 camera that means **cropping the top and bottom away**, and the field of view thrown off the screen is field of view the Collector is about to record. For a product whose output is the footage, a preview that hides part of the frame is worse than one with bars.<br><br>**What the device's own camera app does, which settled it.** It contains: the picture keeps its true ratio, fills the full **height**, and leaves black to the left and right — which is where a camera app puts its controls. That is the framing a Collector already recognises, and it shows the whole frame.<br><br>**So `BoxFit.contain` against the full window is deliberate, and this row is the departure.** The chapter is not satisfied and is not being reported as satisfied.<br><br>**The other half of the fix, which is NOT a departure.** `SafeArea` wrapped the preview, so its box measured **753x320 inside a 793x360 window** — 40 logical points taken on each axis for the status bar, the navigation bar and the display cutout. That produced bars on all four sides and a needlessly small picture, and **no fit setting could have helped**, because the slot was never the screen. `SafeArea` now wraps only the controls, which are the things that must not sit under a notch. **The height is genuinely full for the first time**, and the bars that remain are the aspect ratio's alone.<br><br>**Cost, and what it does not touch.** The Recording Screen shows a 16:9 picture on a 2.2:1 display, so roughly 20 % of the screen's width is black. **The recorded file is unaffected** — verified on a clip pulled byte-exact from the handset after this change: coded `avc1` **1920x1080**, `tkhd` matrix `(1, 0, 0, 1)`, rotation **0°**, identical to every prior check. `buildPreview` is a display widget over a texture id and holds no `CameraController`, so ADR-053's seam makes influencing capture structurally impossible rather than merely unlikely.<br><br>**What would reopen this.** A Collector-facing reason to see the frame edge-to-edge — a framing aid, or a device whose screen is close enough to 16:9 that covering crops almost nothing. Neither exists today, and the chapter should be corrected at its source to describe contained framing rather than left to disagree with the code. | Ch 5.1 §1, Ch 2.7 C-09, ADR-053, Mission 8.2A |
...
### A-230 — the client and the server believed different sources for the same field

Mission 7.8, scoped as *"client-side role-based UI gating matching the backend's Chapter 4.8 Authorization Design."*

**The gating already existed.** ADR-037 built it in Mission 2.7: the route tree is partitioned into `/admin/*` and `/collector/*` and one guard polices the boundary. Every role check in the entire application is six lines in `auth_guard.dart` — no screen, no button, no action checks a role, and per-route checks were explicitly rejected because *"per-route callbacks would repeat the same predicate 19 times and fail open on the twentieth."* Client gating matched backend enforcement structurally before this mission opened.

What did not match was where each side got the role from.

#### ADR-048's argument, unapplied to the second field

`POST /v1/auth/verify` has always returned `{userId, orgId, role}`. `_fetchSession` read the first two and **discarded the third**; `_toUser` read `role` from the Firebase custom claim instead.

The backend does not. `Caller.role` is resolved by the authorizer from the `users` table and documented there in as many words: *"Authoritative role from the users table, not the token claim."*

So the client believed the claim and the server believed the table. A role changed in one and not the other put them out of step until the next token refresh — the app offering Admin surfaces whose every write the backend refused.

**ADR-048 made exactly this argument for `org_id`** — *"a claim written once goes stale the moment an account moves organisation"* — and did not extend it to `role`. Same argument, second field, four missions apart.

**Severity, stated honestly rather than dramatically.** Not a breach: the backend stays authoritative, so divergence produces 403s, not access. And nothing today changes a role — `redeem` writes `collector` from a literal, nothing else writes `users.role`, and the Mission 6 gap register records admin as a manual bootstrap. **The divergence is currently unreachable**, and becomes reachable the moment anyone edits `users.role` by hand without also setting the claim — which is precisely how an admin is made today.

**The refusal moved rather than weakened.** The client's claim check was deleted, not replaced: `provisionCaller` refuses an account with no usable role claim server-side, so such an account never receives a session at all. What changed is which side says no.

Two mirror tests pin it, modelled on the `org_id` test ADR-048 left behind: an admin claim with a collector table yields collector, and the reverse yields admin. Both directions, because under-privileging is the safer failure and still a failure.

#### A diagnostic string was reaching users, and the fallback that hid it almost never ran

Tracing what a 403 *does* on screen — because the role fix makes that case rarer without making it impossible — found the write banner rendering `failure.message`, with *"Check your connection and try again."* as the fallback.

**The fallback almost never fired.** `VumpApi._named` rebuilds a refused request's exception from the Chapter 4.6 envelope, so the message was populated for anything the backend named. What an Admin actually read was:
...
`Failure`'s own documentation says the opposite should happen: *"Presentation should prefer text resolved from `code`, since only that is localisable."* The screen preferred the message.

**Two things made this survive.** The comment above it justified rendering the message — *"ValidationException carries which field, which is more useful than anything this widget could invent"* — describing a case that essentially never occurred while the common case went undescribed. And **the banner had no test at all**, so nothing ever asserted what it displayed.

`classifyWriteFailure` now mirrors `classifyReadFailure` at the same layer, and the message is not rendered at all. Two cases, matching reads' restraint: the question is not how many ways a write can fail but **how many different things the person can usefully do**. `notPermitted` deliberately carries no *"try again"*, since retrying is exactly what will not work.

#### The shape worth carrying

Both findings are the same one at different scales: **a value that two components must agree on, where nothing checks that they do.**

| | The two sides | What made them disagree |
|---|---|---|
| **A-222** | GCP attribute mapping / IAM binding | one carried a session suffix, the other did not |
| **A-228** | `package.json` / `package-lock.json` | a workspace added to one and not the other |
| **A-230** | client role source / server role source | a claim written once, a table read every time |

A-222 and A-228 were caught by an apply and by CI. This one was caught by reading, because **nothing executes the comparison** — the two sources agree in practice today and would diverge silently on the day they stopped.

#### Also corrected

ADR-037's consequence justifying the role guard cited two supports and had lost one: *"the Firestore rules check the admin claim (ADR-036)"*, deleted by Mission 7.6. The claim is unchanged and the argument holds — the Firestore rules governed `org_invite_codes` writes and never gated `/admin` — but the loss is recorded rather than edited away, because a reader meeting one support cannot otherwise tell whether it was always sufficient alone.

A doc comment orphaned since Mission 7.2 also came out: *"The claim carrying the user's organisation"*, left with no constant beneath it when ADR-048 removed `_orgClaim`. The same residue this mission would have created for role, left behind by the change this one mirrors.

#### Not done

**No device verification.** Nothing here is device-dependent, and the one scenario a device could prove — a demoted admin losing Admin surfaces — requires manufacturing a state the product cannot currently produce. Recorded as skipped for that reason rather than omitted silently.

---

### A-231 — the backend's coverage had never been measured, and the first measurement was wrong

Mission 7.9 closes gap register item 15, open since ADR-045 recorded a coverage gate as *"deliberately absent — revisit at 6.3"* and deferred it through 6.3, 6.4, 6.5, 6.6 and 6.7.

Three findings, in the order they were forced.

#### The tooling was configured and had never worked

`vitest.config.ts` has carried `coverage: { provider: 'v8', reporter: ['text-summary'], include: [...] }` since Mission 6.2. **`@vitest/coverage-v8` was never in `devDependencies`**, so:

```
npx vitest run --coverage  →  MISSING DEPENDENCY  Cannot find dependency '@vitest/coverage-v8'
```

The configuration ADR-045 describes as *"Coverage is reported by `vitest run --coverage`"* was **unusable from the day it was written**. Nobody could have run it, and nobody did — across six missions in which the sentence was read and believed.

**This is a configuration that documents an intent rather than a behaviour**, which is the same shape as a comment describing a state the code no longer has. It is harder to see: a comment can be checked by reading, while this could only be checked by running the command, and the command was never part of any routine.

#### The first measurement understated coverage by 8.7 points, and its zeroes were fictional

With the provider installed, the repository read **66.33% lines**, and six files read **0%** — `authorizer`, `caller`, `org`, `request`, `row`, `transaction`.

Several of those are executed by every passing test. `row.ts` is read by every handler that touches a database row. The cause is structural: **`@vump/shared`'s `main` is `./dist/index.js`**, so a function test importing `@vump/shared` runs compiled JavaScript, while coverage's `include` names `packages/*/src/**/*.ts`. All of that execution landed on files coverage was not watching.

Every file reading 0% was one with **no co-located test** — not one nothing executed. Adding a `resolve.alias` to source:
...
| `transaction.ts` | 0% | 88% |
| `handler.ts` | 73% | 100% |
| **Repository** | **66.33%** | **75.02%** |

**A gate set on the first numbers would have been a gate on a measurement error** — failing files that are well covered, and passing a codebase whose real figure it had never seen. The number was available and wrong, which is worse than absent: absent invites measurement, wrong invites action.

#### Vitest cannot express the gate, and that was established by running it

The gate wanted two numbers: a repository floor and a per-file floor with named exemptions. Vitest carries **one**, read as a total or — with `perFile` — as a uniform per-file floor.

Glob entries look like the way out. They are not, and this was measured rather than reasoned about: a glob threshold at `lines: 50` over `packages/shared/src/**.ts` **passed** while `caller.ts` sat at 0% and `authorizer.ts` at 9%, because **a glob threshold is checked against the aggregate of the files it matches**. The group average carried the two zeroes. Glob entries also cannot carry `perFile` — the type is `Pick<Thresholds, …>` without it.

So the gate is `scripts/check-coverage.mjs`: 70% repository, 50% per file, four exemptions each with a target and a reason, printed on every run.

**An exemption is a floor of its own.** A file that rises above its target *fails* the gate until its entry is removed, so the list cannot silently become permanent — the failure mode a list of dispensations otherwise has. All three branches were proven by deliberately breaking them, per the standard A-067 set.

#### Two entry-point scripts are excluded rather than exempted

`cli.ts` and `bootstrap.ts` both end in `await main()`, so **importing them runs them** — `naming.ts` exists precisely because of that and says so: *"re-exporting from it would run the bootstrap as a side effect of importing."* They are scripts, not modules.

They are removed from the denominator entirely, not given exemptions. Leaving 63 uncoverable lines in would make the repository figure partly a measure of how much entry-point code exists. The global reads **82.94%** on that accounting rather than 77.72%, and the difference is stated here because both numbers appear in this mission's commits.

#### What the gate found, and what was fixed

Four files were raised rather than exempted: `org.ts` 37.5% → **100%**, `naming.ts` 0% → **100%**, `auth.ts` 38.88% → **100%**, `firebase.ts` 6.45% → **33.33%**.

`auth.ts` is the one worth naming. Its uncovered half was `verifyToken` and the memoised Admin app, and `handler.test.ts` mocks `./auth.js` wholesale — so **nothing anywhere exercised the claim narrowing, the error conversion or the memoisation.** The role claim is attacker-adjacent, arriving in a token, and a number or object reaching `provisionCaller` as a role would be a type nothing checks again.

`firebase.ts` required exporting `externalAccountOptions` to be testable, which is the point rather than a concession: every field it builds is one whose wrongness fails as a 403 naming neither side — [[A-222]] and [[A-223]] each cost a bisection to find one. The tests pin the impersonation URL, the lower-case `aws1` the library's regex requires against the `AWS1` Google documents, and the **absence** of `region_url` and `url`, which is what makes Lambda work.

#### A Node behaviour worth recording on its own

**`process.env[k] = undefined` sets the string `"undefined"`. It does not unset.**

Three tests asserting that a missing environment variable is refused all passed **vacuously** until they used `Reflect.deleteProperty` — `requireEnv` was receiving `"undefined"`, a perfectly good non-empty string, and never threw. They were caught because they *failed* first, for an unrelated reason; had they been written the other way round they would have passed and proven nothing.

That assignment idiom appears in this repository's `afterEach` blocks, where it works only because each `beforeEach` overwrites the key again. It is not wrong there. It is wrong anywhere a test depends on a variable being *absent*.

**The instance is fixed and the class is not, and the class is the finding.** These three passed while asserting nothing, and the only reason anyone knows is that they failed first for an unrelated reason. Written the other way round they would have been green from the day they were added, and would have counted toward the 250 this mission reports.

That shape has appeared before under different mechanisms — [[A-212]]'s mock that could not reject, [[A-224]]'s fake that accepted any payload — and there is still nothing in either suite that would detect it. **A green suite is the strongest evidence this project has, and it is exactly the evidence this defect hides inside.** Recorded as open items 128 and 129, the second asking for a deliberate sweep rather than vigilance.

---

### A-232 — the closing gate, and what a gate that re-runs CI is worth

Mission 7.10 is the fixed verification gate that follows Mission 7's work. Its first question was what it is *for*, given that 7.6–7.9 each closed with real verification of their own and all four had already merged.

**Re-running the suite was not the answer.** Every one of those merged through a PR whose checks run on the **merge result**, so PR #23's thirteen green checks tested 7.9 on top of 7.8, 7.7 and 7.6. A gate whose output is *"the suite is green"* would have duplicated a machine that had already said so more recently — and would be exactly the activity-that-looks-like-verification this project keeps recording.

So the baseline was taken as a floor, not as the mission, and the mission was four things nothing covered.

#### The baseline, on merged develop at `797e301`

`flutter analyze` clean · **1140** tests, 3 skipped and all deliberate · APK builds · `tsc` clean · `eslint` clean · **250** backend tests · coverage **82.94%**, gate passed · `terraform fmt`/`validate` clean.

**Suppressions swept rather than assumed**: five `// ignore:` in `mobile/lib` — three of them in generated `firebase_options_*.dart` — one silenced analyzer rule (`use_null_aware_elements`, invariant I49, unsatisfiable by the toolchain), one `eslint-disable-next-line` in a test, and three skipped tests, two CI-only goldens and one carrying a *"delete this skip when open item 108 closes"* note. Nothing undocumented in either tree.

#### CI never built the APK

Thirteen jobs and `flutter build` in none of them. Every mission from 7.1 onward closed by building the APK **once, locally, on one machine with a warm Gradle cache** — and [[A-205]] is a defect in precisely that command: `flutter build apk --debug` reported success while reading a stale artefact, because only `flutter-apk/` had been emptied and never `apk/`.

**A local build cannot distinguish "compiled" from "found something already there."** A clean checkout can. The `Android build` job now does it on every push, in its own job rather than inside `Test`, for A-095's reason: a build failure and a test failure must be reported by different checks or the reader goes looking for the wrong kind of problem.

#### Two verification debts had fallen out of every register

**F4** — the foreground service surviving a genuine screen-off window — is still unproven. [[A-218]] records two attempts that established nothing, and A-215's qualifier stands. It was written into an amendment and **nowhere else**: amendments are history, the open-items register is live state, and someone asking *"what is still unverified"* reads the second. That is how it passed through 7.6, 7.7, 7.8 and 7.9 untouched. Now open item 130.

**F5 is worse and is the more useful finding.** Mission 7.5 addressed five gaps; four produced amendments (A-216, A-217, A-218, A-219) and **the fifth produced nothing.** Searched exhaustively: `docs/` names Mission 7.5 in one file, no document enumerates its gaps, and every other `F5` in the repository belongs to a different mission — the F-numbers are per-mission and reused, which is what let it hide.

**So F5 exists only in conversation.** Neither the project owner nor the assistant could recall its subject, and open item 131 is therefore **closed as lost** rather than left standing — an open item asserts someone is expected to resolve it, and nobody is going to.

#### The throughline, which is the part worth keeping

F5 is open item 115's shape exactly: *"the batch existed only in conversation, which is the failure this register exists to prevent."* That item was raised at Mission 7.3, four missions before this one.

**And it recurred inside the mission whose entire purpose was closing verification gaps.** Mission 7.5 existed to convert unverified claims into evidence; one fifth of its own subject matter was never written down and is now unrecoverable.

Three properties made it invisible, and each is cheap to break:

- **The F-numbers are per-mission and reused.** `F5` appears in Missions 7.2, 7.4 and 7.5 meaning three unrelated things, so a search for it returns hits that look like answers.
- **Four of five produced amendments**, so the set looked complete from any angle except counting — and nothing states the count.
- **A gap that produces no artefact leaves no gap-shaped hole.** A-218 exists precisely because F4 *failed*; F5 produced neither success nor failure, and silence is indistinguishable from absence.

**The rule this leaves:** a mission that enumerates its own scope should write that enumeration into the repository before working on it, not after. Every one of Mission 7.6's six phases is recorded because the phases were listed in a handoff first. Mission 7.5's five gaps were listed in conversation only, and the one that produced nothing is the one that vanished.

#### The mutation sample: 18 of 20 killed, and the two survivors differ in kind

Open item 129 says nothing detects a test that passes while asserting nothing. A gate reporting *"1,390 tests pass"* is the exact evidence that item says is unreliable, so twenty tests were sampled against it.

**Selected deliberately, not randomly**: ranked by last-touched date then commit count, oldest and least-revisited first — fourteen mobile from 2026-08-09 to 08-15, six backend from 08-18. **Everything Missions 7.6–7.9 wrote was excluded**, because those were reviewed this week and including them would bias toward a clean result.

Each mutation targeted the specific claim in the test's own name, because a survivor from a carelessly chosen mutation proves nothing — that would be item 129's failure mode reproduced inside its own investigation.

**Eighteen killed.** Two survived, and separating them is the point:

**`free_space_channel` — a coverage gap, not vacuity.** The test declares its own `MethodChannel('vump/free_space')` and injects it everywhere, so the production default is never exercised and its name is never checked. Every test name claims something about request, response or error conversion: **the tests test exactly what they say they test.** The gap is that the channel name exists as two independent literals with nothing tying them — open item 126's shape.

**`pagination` — a tautological assertion, and the closest thing to item 129 found.** `expect(parsePageRequest(null)).toEqual({ limit: DEFAULT_LIMIT })` imports the constant **from the module under test** and asserts the module agrees with itself. Any value survives.

A control makes that precise rather than rhetorical: mutating `MAX_LIMIT` in the same file **was** killed, because a sibling test asserts the error text `/between 1 and 200/` and hard-codes the number. **One constant is pinned and the other is not, and the difference is whether an assertion names a value or borrows it** — an asymmetry that arrived by accident, not design.

Both are fixed rather than registered, and both fixes were verified by re-running the original mutation: **now killed.** The channel test uses the default constructor, so the two literals are tied by behaviour since the private field cannot be read; the pagination test pins both limits to their literal values.

#### What this settles about item 129, and what it does not
...
### A-234 — A-096 reaffirmed: device verification remains the standard for `isar_chunk_store.dart`

**Date: 2026-08-22, Mission 8.1. Reaffirmation, not a new decision — A-096 stands unamended.**

Mission 8.1's scope is closing Chapter 9.5's targets, which makes the one file that cannot meet them a question the sub-mission has to answer before it starts rather than after. **It is answered the same way, and the three conditions A-096 set for reopening itself were tested rather than assumed.** None has occurred:

| A-096's reopening condition | Status at Mission 8.1 |
|---|---|
| ADR-038 / item 13's maintenance risk materialising | **Not triggered.** Item 13 is open and unchanged; `isar: ^3.1.0+1` is the same constraint A-096 was written against |
| A defect escaping to a device that a unit test would have caught | **Not triggered.** Every amendment after A-096 that mentions Isar — A-113, A-117, A-127, A-137, A-198, A-208 — is a different subject |
| Isar shipping a test-host binary needing no download | **Not triggered.** Same pinned version |

**`Isar.initializeIsarCore(download: true)` remains declined**, for A-096's reasons unchanged: it puts a network dependency into a job that has none, adds a recurring failure mode to every CI run and every offline developer build, and introduces a downloaded binary nobody has reviewed — a permanent operational cost to move one file's number.

**The numbers moved and the shape did not.** A-096 measured the layer at 68.3% (505/739) with the file at 1.9% (4/212). At Mission 8.1's trace it read 70.6% (584/827) with the file at **1.8%** (4/220) — the file grew eight lines and gained no covered line. After this sub-mission's tests landed the layer reads **72.5%** (642/886) and the file is unchanged at 4/220. Excluding it, the layer reads **95.8%**.

**What this reaffirmation adds is a correction to the denominator**, recorded separately as [[A-235]]. A-096's arithmetic and every published figure since computed the layer from `lcov.info`, which omits files no test loads — and eight data-layer files were omitted on that basis. Seven of them now have tests. So *"the miss is one file"* was true of the measured set and not of the layer, and it is now true of both.

Chapter 9.5 §2's 80% figure is not amended down. The target stands and is recorded as missed; what is amended is nothing — A-096 already said the gap was chosen rather than suffered, and it still is.

---

### A-235 — the published data-layer coverage figure was measured over 33 of 41 files

**Date: 2026-08-22, Mission 8.1. A correction to `mission-7-report.pdf` §6 and to the Feature Tracker's coverage caveat.**

**`mission-7-report.pdf` states the mobile data layer at 70.6% (584/827) against Chapter 9.5 §2's 80% target.** That number is arithmetically correct and its denominator is not the layer. It is the 33 data-layer files `lcov.info` listed. **Eight more existed, holding 596 source lines, and none appeared in the report at any percentage.**

`testing-standards.md` already recorded the mechanism, at the line the trace found it: *"`lcov.info` lists only files loaded during the run, so a file with no test does not appear as 0%, it does not appear at all."* The sentence was written at Mission 2.8 about a different measurement and was true of every measurement since, including three published in Mission 7's status report.

**This is [[A-231]]'s shape on the mobile side.** There, `@vump/shared`'s `main` pointed at `dist/`, so a function test importing it ran compiled JavaScript while coverage watched `src/` — and six files read 0% that were executed by every passing test. Both are a measurement whose denominator is not what the reader believes it is, and in both cases **the number was available and wrong, which is worse than absent: absent invites measurement, wrong invites action.**

The direction differs and matters. A-231's error *understated* real coverage. This one *overstated* it, because the missing files were the untested ones.

**Corrected figures.** With seven of the eight now tested, the layer reads **72.5% (642/886) over 39 of 41 files**. The two still outside it:

- **`foreground_upload_service_host.dart`** — 188 lines, no test. Out of scope for Mission 8.1 by the project owner's ruling, to be scoped with 8.3 given that open item 130 records the same component's screen-off behaviour as unproven. **It is the one component in the project with neither a unit test nor a discharged device verification.**
- **`recording_schemas.dart`** — now tested, and it will never appear in a coverage report regardless. It is a `static const` list, which compiles to no executable statements, so there is nothing to instrument. **Tested and measurable are different properties**, and a file can be the first without ever being the second.

**What this leaves.** The correction is recorded rather than quietly fixed because the wrong figure was published in a delivered artefact, and because a percentage whose denominator nobody checks is exactly the kind of evidence this project keeps finding it has trusted. Repository-wide the same gap remains: **252 hand-written files in `lib/`, 168 in `lcov`** after this sub-mission — up from 162, so seven new files entered the measurement and one (`recording_schemas.dart`) cannot. That is not a Chapter 9.5 target and nothing gates it, which is why it is stated here rather than left to be rediscovered.

---

### A-236 — an exemption ceiling estimated from outside a file, and wrong by sixty points

**Date: 2026-08-22, Mission 8.1. A finding about how coverage exemptions get their numbers, not about any one file's coverage.**

Mission 7.9's gate gave four files an exemption, each with a target and a reason printed on every run. Three targets were **effort estimates** and said so — 251 lines, Data API fixtures, work the mission chose not to take on in one session. **The fourth was different in kind, and its difference was the defect.**

`functions/redeem/src/firebase.ts` was exempted at **40% rather than 50%**, and the reason recorded was:

> *"Mission 7.9 tested the pure half (6.5% to 33%). The remainder is the live token exchange, which no unit test can reach — 40 rather than 50 because **the ceiling here is real, not a matter of effort**."*

**It reached 100%.**

#### What the estimate got right, and what it assumed

The token exchange itself is still unreachable, and no test added at 8.1 attempts it. That half of the claim holds.

What was wrong is the inference from it. `federatedCredential()` and `firebaseApp()` are **not exported**, and the estimate treated *"unexported, and wrapped around an unreachable call"* as *"unreachable"*. It is not the same thing:

- **`firebaseApp()`** memoises, adopts an app the runtime already holds under the same name, and refuses when `ExternalAccountClient.fromJSON` returns `null` rather than throwing. Three branches, all reachable through the exported `federatedAuth()`.
- **`federatedCredential()`** builds the credential object, and its `getAccessToken` closure carries **expiry arithmetic** — the 60-second skew, and a `Math.max(1, …)` floor. That closure was reached without exporting anything, by capturing the credential from the object `initializeApp` receives.

**The arithmetic is the part that would have failed silently.** An `expires_in` computed without the skew hands firebase-admin a token it believes is valid a minute longer than it is, and the symptom is an intermittent 401 on the route that provisions accounts — the same class of failure as [[A-222]] and [[A-223]], where a wrong value is indistinguishable from a right one until Google refuses it.

#### The general finding

**A ceiling is a claim about what cannot be tested, and it was made without trying.** The other three exemptions were honest about being estimates of *effort*, which is a claim about a mission's time and is checkable by spending it. A ceiling is a claim about the *code*, and the only way to check it is to attempt the test and fail.

That distinction matters because of how the two decay. An effort estimate is visibly a debt and the gate prints it on every run; open item 127 tracked exactly that and is now closed. **A ceiling reads as a fact, so nobody re-examines it** — and this one would have stood indefinitely, at a number 60 points below what the file could reach, with the gate reporting green the whole time.

It is [[A-231]]'s shape once more, and closer to it than [[A-235]] is. A-231 records a configuration *"that documents an intent rather than a behaviour"*, harder to see than a stale comment because *"a comment can be checked by reading, while this could only be checked by running the command, and the command was never part of any routine."* An exemption ceiling is the same: it can only be checked by writing the test it says is pointless.
