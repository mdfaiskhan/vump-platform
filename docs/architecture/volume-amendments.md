...
| 48 | **No CI run has ever executed against this repository's work — the pipeline has never once run** | **Found 2026-08-16 by Mission 4.3.5, trying to obtain the first real confirmation.** Three facts compound. (1) `ci.yml`'s `push` trigger covers `main`, `develop`, `release/**`, `hotfix/**` — **not** `mission-0.18.4-ci`, so none of the ~78 commits pushed to that branch has triggered anything. (2) `workflow_dispatch` cannot be used: GitHub only registers it when the workflow exists on the **default branch**, and `.github/workflows/ci.yml` **does not exist on `origin/main` at all** — `main` is 78 commits behind and predates the workflow. (3) `pull_request` is scoped to `[main, develop]`, and **`develop` does not exist on the remote** — the only branches are `main` and `mission-0.18.4-ci`, which is A-022's missing-`develop` gap showing up as a broken trigger rather than as a topology question. **The only route to a first run is a PR from `mission-0.18.4-ci` into `main`.** This is the largest instance yet of *"a green check over an empty set is not evidence"* (items 23, 26, 32, 41, 42): every CI job in this project has been reasoned about, edited and cited across a dozen missions while never having executed. Two jobs were found red by local replay alone (items 42, 47); nothing establishes the other seven have ever passed. | Mission 4.3.5, A-022, ADR-019 |
