...
- **A test per mapped code.** `ErrorInterceptor.mapToNetworkException` is a static specifically so this is possible without a live client. ~~13 documented mappings (§8) means 13 cases, and there is currently no test file for it.~~ **Closed in Mission 2.8:** `test/core/network/interceptors/error_interceptor_test.dart` covers all 13, plus the ordering property that 429 and 5xx are matched before the `>= 400` range that would otherwise swallow them. `FirebaseAuthErrorMapper` (19 codes) and `FirebaseFunctionsErrorMapper` are covered on the same rule.
...
| ~~**No test for `ErrorInterceptor.mapToNetworkException`**~~ | ~~13 documented mappings, no test file~~ | **Closed, Mission 2.8.** `test/core/network/interceptors/error_interceptor_test.dart` exists and covers all 13 mappings, what the exception carries, and the guard-clause ordering |
