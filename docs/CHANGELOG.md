...
- **2026-08-18** — **Firebase is three projects, and the build flavor picks one.** `vump-dev`, `vump-staging` and `vump-prod` replace the single `vump-platform-f86af`, each with its own Firestore in `asia-south1` and delete protection on. Volume 7 Chapter 7.7 §1 asked for this so that a dev build cannot touch production data; deferred item 3 is closed.
  ...
  Verified on a real device rather than from configuration: a `--flavor dev` build on the CPH2707 logs `Firebase initialised for Development (project "vump-dev")`. Mission 6.4.
