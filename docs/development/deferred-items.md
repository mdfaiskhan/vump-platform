...
| 3 | ~~One Firebase project serves all three environments~~ — **CLOSED, Mission 6.4.** `vump-dev`, `vump-staging` and `vump-prod` exist, each with its own Firestore in `asia-south1`, and the build flavor selects between them | Closed | Three `android/app/src/{flavor}/google-services.json`, verified project IDs. ADR-047, Amendment A-162 |
...
| 10 | `functions/` is a Firebase Cloud Function that ADR-036 marks **temporary**, *"retired at Mission 6/7"*. Mission 6.2 built the real backend and did **not** retire it | Mission 6.5, or the first mission that writes a Firebase custom claim | ADR-036 line 122: *"No CI check can detect that Mission 6/7 has happened"*. `functions/src/index.ts` still deployed |
...
| 12 | Every existing Firebase account carries `org_id: "vump-default"`, a literal string. Mission 6.3 defines `users.org_id uuid NOT NULL REFERENCES orgs(id)`, so the claim value cannot be represented in the schema and matches no row | Whichever mission ports `redeemInviteCode` and implements A-159's backend half — likely 6.5 | `functions/src/index.ts` `DEFAULT_ORG_ID = "vump-default"`; migration `0002_orgs_users.sql`. Amendments A-162, A-163 |
