...
| 13 | Firebase **Auth is not enabled** in any of the three new projects, and **billing is not linked**, so `redeemInviteCode` is not deployed to them and the Artifact Registry cleanup policy is not replicated. Both are console-only owner actions <br><br>**The `redeemInviteCode` half is void since Mission 7.6 Phase 6** (A-227): the function is undeployed and deleted everywhere, so it is not pending deployment to staging or prod — there is nothing to deploy. The Artifact Registry cleanup policy goes with it. **What still stands is Auth and billing**, which `firebase_auth` needs on its own account, independently of the retired runtime — so this item narrows rather than closes. | Project owner, before Mission 6.4 can close | `accounts:signInWithPassword` → `CONFIGURATION_NOT_FOUND`; `firebase deploy --only functions` → *"must be on the Blaze plan"*. Amendment A-162 |
