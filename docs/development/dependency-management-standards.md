...
**~~Two entries are marked temporary~~ — both are gone, and the prediction held.** `cloud_functions` and `cloud_firestore` existed only to reach the invite-code runtime ADR-036 stood up, and both left the project when that runtime was retired at **Mission 7.6 Phase 6**. Redemption is now `POST /v1/auth/redeem`, carried by `dio`, which was already here.

They were classed Leaf rather than Core deliberately, on the reasoning that *"nothing architectural depends on them, and their removal should be a deletion rather than a migration."* **It was a deletion.** `cloud_firestore` had exactly one consumer at retirement and `cloud_functions` had none; both came out in a single edit. See A-227.
