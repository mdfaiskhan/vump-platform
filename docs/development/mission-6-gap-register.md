...
| 4 | **Nothing issues the `admin` claim.** `firestore.rules` required it to write `org_invite_codes`, and `redeemInviteCode` set `role: "collector"` from a literal on every path — so that collection was writable by nobody, and admin is an undocumented manual bootstrap.<br><br>**Half of this is void since Mission 7.6 Phase 6**: the rules file, the collection and the function are all deleted, and the issuing surface they gated was retired rather than ported (A-227). **The other half stands and is the part that mattered** — nothing still issues the `admin` claim, `POST /v1/auth/redeem` hard-codes `collector` at one site, and admin remains a manual bootstrap performed outside the application. | Unreachable capability | A-158, ADR-036, A-227 |
...
| ~~7~~ | ~~**`functions/` is not retired** and claim-writing stays in Cloud Functions~~ **CLOSED by Mission 7.6.** The gap was stated as deliberate *"with a trigger"*, the trigger being that porting moves the claim writer outside Google and forces the credential ADR-036 deferred. **The port happened and the credential did not.** Workload Identity Federation removed the need for a key rather than accepting one, so the reason this gap stayed open turned out to be avoidable rather than a cost to pay (A-221). `functions/` is deleted and `redeemInviteCode` is undeployed. | **Closed** | A-165, A-221, A-227 |
| 8 | **The BR-08/11/21/22 behavioural proofs exist only as an uncommitted scratchpad script.** **Still open after ADR-049, and the reason has moved rather than gone:** the credential exists (`vump-dev-ci-db-prover`) and no CI job runs the proofs. Migration `0007` grants `DELETE` to nobody, so they cannot tear down. **This row said a read-only principal would close it; that was wrong — the proofs write.** **Two of the three proof shapes now have a demonstrated teardown-free method** (A-187's negative probes write nothing; A-193's seed-and-rollback commits nothing), both exercised live at Mission 7.3. The third — "role X cannot touch role Z's row" — still needs two sessions and remains blocked by A-173. Gap unchanged: the credential exists, no CI job runs the proofs | Test coverage | 6.3.1 close-out, **A-172**, **A-173**, **A-193** |
...
| ~~15~~ | ~~**The backend has no coverage gate.**~~ **CLOSED by Mission 7.9.** ADR-045 recorded it as "deliberately absent — revisit at 6.3", and it was deferred through 6.3, 6.4, 6.5, 6.6 and 6.7 — five missions past its own trigger.<br><br>**70% repository, 50% per file**, in `backend/scripts/check-coverage.mjs`, gating the `Backend` CI job. Four named exemptions carried forward as open item 127.<br><br>**Closing it found that coverage had never been measured at all**: `@vitest/coverage-v8` was never installed, so the configured provider answered MISSING DEPENDENCY from Mission 6.2 onward. The first real measurement was then wrong by 8.7 points through cross-package misattribution. A-231. | **Closed** | ADR-045, A-231 |
