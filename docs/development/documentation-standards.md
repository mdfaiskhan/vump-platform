...
Counts in this document are re-verified when it is edited, per §30. They were last verified at **50 markdown files and 30 ADRs**.
...
Two sections are not in that template but have been used by every ADR since ADR-001 and are now equally expected: **`Related Missions`** and **`Implementation Status`**. All 30 ADRs carry `Related Missions`; 29 carry `Implementation Status` — ADR-002 is the sole exception, recorded in §30.
...
Verified, and unanimous: **all 30 ADRs contain zero horizontal rules**, while every prose document uses between 3 and 11. The ADR template's headings carry the structure by themselves; a rule between `## Decision` and `## Alternatives Considered` adds a line and no information.
